...
public sealed class DataWorkspacePresentationBuilderTests
{
    [Fact]
    public void Build_WithLiveOperationalTelemetry_ProjectsProviderBackfillStorageAndExportState()
    {
        ...
        var data = new DataWorkspaceData
        {
        ...
        var presentation = DataWorkspacePresentationBuilder.Build(data);
        ...
        presentation.IntegrationWorkflowSteps.Select(step => step.Label)
            .Should().ContainInOrder("Connect Source", "Acquire Data", "Validate Data", "Normalize Data", "Store Data", "Publish Data");
        presentation.IntegrationWorkflowSteps.Select(step => step.Status)
            .Should().ContainInOrder(
                ...
                WorkflowStepStatus.Complete,
                WorkflowStepStatus.Current);
        presentation.IntegrationWorkflowSteps.Select(step => step.StatusLabel)
            .Should().ContainInOrder("Connected", "In progress", "Checked", "Normalized", "Stored", "Publishing");
    ...
    {
        var presentation = DataWorkspacePresentationBuilder.Build(new DataWorkspaceData
        {
        ...
        presentation.IntegrationWorkflowSteps.Select(step => step.Label)
            .Should().ContainInOrder("Connect Source", "Acquire Data", "Validate Data", "Normalize Data", "Store Data", "Publish Data");
        presentation.IntegrationWorkflowSteps[0].Status.Should().Be(WorkflowStepStatus.Current);
        presentation.IntegrationWorkflowSteps[0].StatusLabel.Should().Be("Blocked");
        presentation.IntegrationWorkflowSteps.Skip(1).Select(step => step.Status).Should().OnlyContain(status => status == WorkflowStepStatus.Pending);
