...
public sealed class StrategyWorkspaceShellPageTests
{
    [Fact]
    public void BuildDeskHeroState_WithoutRecordedRuns_PrioritizesBacktestAndWatchlists()
    {
        var hero = StrategyWorkspaceShellPage.BuildDeskHeroState(
    ...
    [Fact]
    public void StrategyWorkspaceShellPageSource_ShouldBindDeskBriefingHeroToViewModel()
    {
        var code = File.ReadAllText(GetRepositoryFilePath(@"src\Meridian.Wpf\Views\StrategyWorkspaceShellPage.xaml.cs"));
        var xaml = File.ReadAllText(GetRepositoryFilePath(@"src\Meridian.Wpf\Views\StrategyWorkspaceShellPage.xaml"));
        ...
        var models = File.ReadAllText(GetRepositoryFilePath(@"src\Meridian.Wpf\Models\StrategyWorkspaceShellPresentationModels.cs"));
        ...
        xaml.Should().Contain("StrategyHeroBadgeText");
        xaml.Should().Contain("StrategyHeroPrimaryActionButton");
        xaml.Should().Contain("StrategyHeroSecondaryActionButton");
        ...
        xaml.Should().Contain("Text=\"{Binding StrategyWorkflowStatusText}\"");
        xaml.Should().Contain("ItemsSource=\"{Binding BriefingInsights}\"");
        xaml.Should().Contain("ItemsSource=\"{Binding RecentRuns}\"");

        code.Should().Contain("OnStrategyHeroPrimaryActionClick");
        ...
        models.Should().Contain("internal readonly record struct StrategyDeskHeroState(");
        service.Should().Contain("internal static StrategyDeskHeroState BuildDeskHeroState(");
        service.Should().Contain("internal async Task<StrategyWorkspaceShellPresentationState> BuildAsync(");
        service.Should().Contain("WorkspaceCopyCatalog.Strategy.ShellTitle");
        service.Should().NotContain("WorkspaceCopyCatalog.Research.");
        service.Should().NotContain("ResearchActiveRunPresentation");
        service.Should().Contain("StrategyActiveRunPresentation");
