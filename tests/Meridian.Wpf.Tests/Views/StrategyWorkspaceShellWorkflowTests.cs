...
public sealed class StrategyWorkspaceShellWorkflowTests
{
    [Fact]
    public void StrategyWorkspaceShell_BriefingOpenAction_ShouldHydrateRunStudioAndDockInspectors()
    {
        WpfTestThread.Run(async () =>
        {
            ...
            var tempRoot = Path.Combine(Path.GetTempPath(), "strategy-shell-ui-" + Guid.NewGuid().ToString("N"));
            ...
            services.RemoveAll(typeof(IStrategyBriefingWorkspaceService));
            services.AddSingleton<IStrategyBriefingWorkspaceService>(new FakeStrategyBriefingWorkspaceService(briefing));
            ...
            var page = serviceProvider.GetRequiredService<StrategyWorkspaceShellPage>();
            ...
            try
            {
                ...
                var viewModel = page.DataContext.Should().BeOfType<StrategyWorkspaceShellViewModel>().Subject;
                ...
                GetRequired<TextBlock>(page, "StrategyHeroFocusText").Text.Should().Be("Promotion queue");
                GetRequired<TextBlock>(page, "StrategyHeroBadgeText").Text.Should().Be("Attention");
                GetRequired<TextBlock>(page, "StrategyWorkflowStatusText").Text.Should().Be("1 run is waiting for trading review.");
                GetRequired<TextBlock>(page, "StrategyWorkflowBlockerLabelText").Text.Should().NotBeNullOrWhiteSpace();
                GetRequired<TextBlock>(page, "StrategyWorkflowBlockerDetailText").Text.Should().NotBeNullOrWhiteSpace();
                GetRequired<Button>(page, "StrategyHeroPrimaryActionButton").Content.Should().Be("Run Browser");
                GetRequired<Button>(page, "StrategyHeroSecondaryActionButton").Content.Should().Be("Open Watchlists");
                GetRequired<TextBlock>(page, "StrategyWorkflowTargetText").Text.Should().Be("Target page: StrategyRuns");
                ...
                GetRequired<TextBlock>(page, "StrategyHeroFocusText").Text.Should().Be("Promotion review");
                GetRequired<TextBlock>(page, "StrategyHeroBadgeText").Text.Should().Be("Ready");
                GetRequired<Button>(page, "StrategyHeroPrimaryActionButton").Content.Should().Be("Open Trading Review");
                GetRequired<Button>(page, "StrategyHeroSecondaryActionButton").Content.Should().Be("Promote to Paper");
                GetRequired<TextBlock>(page, "StrategyWorkflowTargetText").Text.Should().Be("Target page: TradingShell");
                ...
                var openDocumentKeys = GetDockDocumentKeys(GetRequired<MeridianDockingManager>(page, "StrategyDockManager"));
    ...
    private static StrategyBriefingDto BuildBriefing(string runId, string summary)
    {
        var generatedAt = DateTimeOffset.UtcNow.AddMinutes(-4);
        var drillIn = new StrategyRunDrillInLinks(
    ...
    private sealed class FakeStrategyBriefingWorkspaceService : IStrategyBriefingWorkspaceService
    {
        private readonly StrategyBriefingDto _briefing;

        public FakeStrategyBriefingWorkspaceService(StrategyBriefingDto briefing)
            => _briefing = briefing;
