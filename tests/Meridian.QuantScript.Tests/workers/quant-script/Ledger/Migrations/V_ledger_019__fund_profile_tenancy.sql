...
-- guard. tenant_id/company_id are trimmed and blank/'all'/null are normalized to null (matching the
-- registry's Trim()/blank-to-null handling), so a whitespace tenant falls back to the company and only
-- genuinely unscoped funds are skipped (stay claimable).
...
from (
    select lower(trim(fund_profile_id)) as fund_profile_id,
           coalesce(
               nullif(nullif(trim(tenant_id), ''), 'all'),
               nullif(nullif(trim(company_id), ''), 'all')) as owner_tenant,
           coalesce(nullif(trim(company_id), ''), 'all') as company_id,
           row_number() over (
    ...
    where coalesce(
              nullif(nullif(trim(tenant_id), ''), 'all'),
              nullif(nullif(trim(company_id), ''), 'all')) is not null
