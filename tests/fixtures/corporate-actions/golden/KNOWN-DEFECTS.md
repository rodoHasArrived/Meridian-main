...
| CA-DEF-007 | `src/Meridian.FinancialOperations/Ledger/AssetAccountingEventSpineService.cs` (posting candidate draft) against `src/Meridian.Storage/Ledger/LedgerPeriodPostingGuard.cs` | An approved corporate-action case's durable post is refused on PostgreSQL: the spine drafts an instrument-bearing journal (metadata `SecurityId`) without the `securityMasterProvenance` / `securityMasterLineage` tags the period posting guard requires; the case stays `Approved` with no journal written | The spine stamps approved, active Security Master provenance and ledger-mapping lineage resolved from the Security Master record, and the post commits one balanced journal and replays its receipt | W9-CORPACT-011 exit criterion four | PostgreSQL round trip (`CorporateActionAccountingPostgresRoundTripTests`, also `Category=Integration`) |
