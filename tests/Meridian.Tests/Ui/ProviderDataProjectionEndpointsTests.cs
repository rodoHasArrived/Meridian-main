...
public sealed class ProviderDataProjectionEndpointsTests
{
    ...
    private sealed class News : IProviderNewsReadService { public string ProviderFamily => "ib"; public IReadOnlyList<ProviderNewsItem> GetNews() => [new("n1", "Results", DateTimeOffset.UtcNow, Symbol: "MSFT", Source: "ib-news", Url: null, Provenance: Evidence("news-results"))]; public async IAsyncEnumerable<ProviderNewsItem> WatchNewsAsync([System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken = default) { yield break; } }
    private sealed class Calendar : IProviderCalendarReadService { public string ProviderFamily => "ib"; public IReadOnlyList<ProviderCalendarEvent> GetCalendarEvents() => [new("c1", "NYSE", DateTimeOffset.UtcNow, DateTimeOffset.UtcNow.AddHours(1), "halt", Description: null, Provenance: Evidence("calendar-nyse"))]; public async IAsyncEnumerable<ProviderCalendarEvent> WatchCalendarEventsAsync([System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken = default) { yield break; } }
    private sealed class Instruments : IProviderInstrumentDiscoveryReadService { public string ProviderFamily => "ib"; public IReadOnlyList<ProviderInstrumentDiscoveryResult> GetInstruments() => [new("i1", "MSFT", "Microsoft", Exchange: "NASDAQ", AssetClass: "equity", Provenance: Evidence("instrument-msft"))]; public async IAsyncEnumerable<ProviderInstrumentDiscoveryResult> WatchInstrumentsAsync([System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken = default) { yield break; } }

    private static ProviderDataProvenance Evidence(string stableKey) => new("ib", "ib-gateway-1", DateTimeOffset.UtcNow.AddSeconds(-1), DateTimeOffset.UtcNow, "US equities", "market-data", "real-time", "scanner", stableKey, "corr-42", stableKey);
}
