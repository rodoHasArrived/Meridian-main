...
public sealed partial class WorkstationEndpointsTests
{
    ...
    [Fact]
    public async Task MapWorkstationEndpoints_ProviderIntegrationRunDueSync_ResolvesDependencyOutputs()
    {
        var testRoot = CreateProviderIntegrationTestRoot();
        try
        {
            ...
            {
                State = ProviderIntegrationActivationStateDto.Active,
                EnabledCapabilities = [ProviderCapabilityKindDto.Accounts, ProviderCapabilityKindDto.Positions],
                ApprovalEvidenceId = "approval-evidence-run-due"
            };
            await tenantStore.SaveManifestAsync(manifest);
            await tenantStore.SaveConnectionAsync(connection);
            ...
            var response = await client.PostAsJsonAsync(
                ProviderIntegrationRunDueSyncRoute(connection.ConnectionId),
                CreateRunDueSyncRequest(connection, includePathParameters: false),
                ServerJsonOptions);
            var result = await response.Content.ReadFromJsonAsync<ProviderIntegrationRunDueSyncResultDto>(ServerJsonOptions);
            ...
            result.Items.Should().Contain(item =>
                item.Capability == ProviderCapabilityKindDto.Positions &&
                item.Reason == "started-with-dependency" &&
                item.DryRunResult!.RecordsAccepted == 1);
        }
        finally
        {
            DeleteProviderIntegrationTestRoot(testRoot);
    ...
    [Fact]
    public async Task MapWorkstationEndpoints_ProviderIntegrationStagingReview_ReturnsAcceptedRecords()
    {
        var testRoot = CreateProviderIntegrationTestRoot();
        try
        {
            ...
            var review = await client.GetFromJsonAsync<ProviderIntegrationStagingReviewDto>(
                $"{ProviderIntegrationStagingReviewRoute("connection-alpha")}?recentRunLimit=1",
                ServerJsonOptions);

            review.Should().NotBeNull();
            review!.ConnectionId.Should().Be("connection-alpha");
            review.SyncRunIds.Should().ContainSingle().Which.Should().Be("sync-run-new");
            review.TotalStagedRecords.Should().Be(1);
            review.ReadyForReconciliationCount.Should().Be(1);
            review.WarningRecordCount.Should().Be(0);
    ...
    [Fact]
    public async Task MapWorkstationEndpoints_ProviderIntegrationStagingReview_RequiresProviderReadPermission()
    {
        var testRoot = CreateProviderIntegrationTestRoot();
        try
        {
            ...
            var response = await client.GetAsync(ProviderIntegrationStagingReviewRoute("connection-alpha"));
    ...
    [Fact]
    public async Task MapWorkstationEndpoints_ProviderIntegrationPromotionReadiness_ReturnsBlockedRows()
    {
        var testRoot = CreateProviderIntegrationTestRoot();
        try
        {
            ...
            var preview = await client.GetFromJsonAsync<ProviderIntegrationPromotionReadinessPreviewDto>(
                $"{ProviderIntegrationPromotionReadinessRoute("connection-alpha")}?recentRunLimit=1",
                ServerJsonOptions);
            ...
            preview.ReadyForReconciliationCount.Should().Be(0);
            preview.Rows.Should().ContainSingle(row =>
                row.StagingRecordId == "staging-sync-run-new-0" &&
    ...
    [Fact]
    public async Task MapWorkstationEndpoints_ProviderIntegrationPromotionReadiness_RequiresProviderReadPermission()
    {
        var testRoot = CreateProviderIntegrationTestRoot();
        try
        {
