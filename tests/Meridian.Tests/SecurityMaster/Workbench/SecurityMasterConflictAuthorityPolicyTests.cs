// BLUEPRINT: tests for Phase 1 of the Security Master Passport Workbench governed-write feature.
...
//
// Real, existing inputs the policy consumes (Meridian.Contracts.Workstation):
//   - SecurityMasterConflictAssessmentDto (CurrentWinningSource, ChallengerSource, RecommendedWinner,
//     ImpactSeverity, IsBulkEligible, Conflict)
//   - InstrumentPassportProviderConfidenceDto (Provider, ConfidenceScore, FreshnessAsOf)
//
// Precedence under test (blueprint D1): golden-copy rule -> source rank -> freshness(AsOf) -> confidence.
