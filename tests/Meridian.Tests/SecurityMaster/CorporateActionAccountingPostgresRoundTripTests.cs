...
public sealed class CorporateActionAccountingPostgresRoundTripTests
{
    ...
    public async Task RetainedCashDividendBinding_AttachApprovePost_ReloadsOneBalancedJournalAndReplaysReceipt()
    {
        ...
        var period = await journal.SavePeriodAsync(new LedgerAccountingPeriod(
            periodId, bookId, PeriodStart.Year, PeriodStart.Month, PeriodStart.ToString("yyyy-MM", CultureInfo.InvariantCulture),
        ...
        Guid securityId, CorporateActionCaseScopeDto scope, CancellationToken ct)
    {
        var action = new CorporateActionDto(Guid.NewGuid(), securityId, CorporateActionEventTypes.Dividend,
            EffectiveDate, EffectiveDate.AddDays(14), 0.24m, "USD", null, null, null, null, null, null, null,
            RecordDate: EffectiveDate.AddDays(1), LifecycleState: CorporateActionLifecycleStates.Confirmed);
