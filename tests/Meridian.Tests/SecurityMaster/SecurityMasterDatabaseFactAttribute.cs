...
public sealed class SecurityMasterDatabaseFactAttribute : FactAttribute
{
    ...
    {
        if (SecurityMasterDatabaseTestEnvironment.TryGetSkipReason(out var reason))
        {
            Skip = reason;
        }
    }
...
internal static class SecurityMasterDatabaseTestEnvironment
{
    ...
    public static bool TryGetSkipReason(out string? reason)
    {
        ...
        {
            reason = $"Security Master PostgreSQL tests are skipped because {DisableDockerVariable}=true. " +
        ...
        reason = "Security Master PostgreSQL tests are skipped because Docker is unavailable. " +
