...
public sealed class FundStructureTenantBackfillTests
{
    ...
    [InlineData(" ALL ", false)]
    [InlineData("tenant-b", false)]
    public void Preview_ExplicitUnscopedLedgerStamp_IsNotMissingEvidence(string? stamp, bool valid)
    {
        var snapshot = MakeSnapshot();
    ...
    [Theory]
    [InlineData("_FundScope", true)]
    [InlineData("FundScope2", true)]
    [InlineData("2bad", false)]
    [InlineData("fund;drop", false)]
    public void BackfillStore_SchemaOverrides_UseSupportedIdentifierSyntax(string schema, bool valid)
    {
    ...
    private sealed class MemoryStore(FundStructureTenantBackfillSnapshot snapshot) : IFundStructureTenantBackfillStore
    {
        ...
        public bool RefuseSnapshot { get; set; }
        public Task<FundStructureTenantBackfillReceipt?> FindReceiptAsync(Guid runId, CancellationToken ct = default)
        {
            ct.ThrowIfCancellationRequested();
            return Task.FromResult(Receipt?.RunId == runId ? Receipt : null);
        }
