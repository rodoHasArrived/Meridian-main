...
$buildStep = Invoke-MeridianStepWithTestHostRetry -Name "Build isolated WPF route slice" -Command $buildCommand -LogName "build.log" -SummaryDir $summaryDir -RepoRoot $repoRoot -Steps $steps -RetryEvents $retryEvents

if ($buildStep.exitCode -eq 0) {
    Invoke-MeridianStepWithTestHostRetry -Name "Run position blotter route test slice" -Command $testCommand -LogName "test.log" -SummaryDir $summaryDir -RepoRoot $repoRoot -Steps $steps -RetryEvents $retryEvents | Out-Null
}
