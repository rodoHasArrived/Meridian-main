...
function Add-StageStatus {
    param(
        ...
        [System.Collections.Generic.List[object]]$StatusEntries,
...
Add-StageStatus -StatusEntries $statusEntries -Stage 'preflight' -Status 'ok' -Message 'Desktop workflow preflight checks completed.'
...
try {
    ...
    if (-not $SkipBuild -and $stageStatuses['build'] -ne 'skipped-valid') {
        ...
        Complete-MeridianCheckpointStep -Context $checkpoint -StepId 'build-desktop' -ArtifactPointers @($exePath)
        Add-StageStatus -StatusEntries $statusEntries -Stage 'build-desktop' -Status 'ok' -Message 'Desktop build completed.' -Metadata @{ exePath = $exePath }
    }
    elseif (-not $SkipBuild) {
        Write-Info 'Skipping desktop build (checkpoint resume).'
        Add-StageStatus -StatusEntries $statusEntries -Stage 'build-desktop' -Status 'skipped' -Message 'Skipped via checkpoint resume.'
    }
    else {
        Add-StageStatus -StatusEntries $statusEntries -Stage 'build-desktop' -Status 'skipped' -Message 'Skipped because -SkipBuild was supplied.'
    }
        ...
        try {
            ...
            if ($existingProcesses.Count -gt 0) {
                ...
                if (Test-MeridianCheckpointStepShouldRun -Context $checkpoint -StepId 'launch-desktop') {
                    Add-StageStatus -StatusEntries $statusEntries -Stage 'launch-desktop' -Status 'running' -Message 'Launching Meridian desktop process.'
                    ...
                    Add-StageStatus -StatusEntries $statusEntries -Stage 'launch-desktop' -Status 'ok' -Message 'Desktop process launched.' -Metadata @{ processId = $ownedProcess.Id }
                    ...
                    Add-StageStatus -StatusEntries $statusEntries -Stage 'launch-desktop' -Status 'skipped' -Message 'Skipped via checkpoint resume.'
                }
            }
    ...
    Add-StageStatus -StatusEntries $statusEntries -Stage 'ensure-operating-context' -Status 'ok' -Message 'Operating context confirmed.'
    ...
    Add-StageStatus -StatusEntries $statusEntries -Stage 'startup-readiness' -Status 'ok' -Message 'Shell readiness confirmed.' -Metadata @{ pageTag = $startupReadiness.State.PageTag; pageTitle = $startupReadiness.State.PageTitle }
    ...
    if ($stageStatuses['capture'] -ne 'skipped-valid') {
        ...
        try {
            ...
            Add-StageStatus -StatusEntries $statusEntries -Stage $checkpointStepId -Status 'running' -Message $title
            ...
            Add-StageStatus -StatusEntries $statusEntries -Stage $checkpointStepId -Status 'ok' -Message $title -Metadata @{ capturePath = $capturePath }
        }
        catch {
            $stepResult.status = 'failed'
            $stepResult.error = $_.Exception.Message
            $stepResult.completedAt = (Get-Date).ToString('o')
            Add-StageStatus -StatusEntries $statusEntries -Stage $checkpointStepId -Status 'failed' -Message $_.Exception.Message -Metadata @{ title = $title; index = $stepIndex }
            try {
                if ($null -ne $window) {
    ...
    Add-StageStatus -StatusEntries $statusEntries -Stage 'workflow' -Status 'failed' -Message $_.Exception.Message
    throw
}
finally {
    ...
    $statusEntries | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath $bundleStageStatusPath -Encoding utf8
