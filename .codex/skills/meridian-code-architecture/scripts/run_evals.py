#!/usr/bin/env python3
"""Run deterministic eval cases for the Meridian code architecture skill."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Run deterministic local fixtures without codex exec. Default unless --live-run is passed.")
    parser.add_argument("--live-run", action="store_true", help="Run live codex exec traces. Use only from an isolated worktree or scratch clone.")
...
def select_cases(cases: list[dict[str, Any]], eval_id: int | None) -> list[dict[str, Any]]:
    ...
    if not selected:
        raise ValueError(f"eval-id {eval_id} not found in {EVALS_JSON}")
    return selected
...
def expand_command(command: list[str]) -> list[str]:
    ...
    return [
        replacements[part] if part in replacements else part.format(repo_root=REPO_ROOT, skill_dir=SKILL_DIR)
        for part in command
    ]
...
def run_live(case: dict[str, Any]) -> EvalResult:
    ...
    checks = [f"trace={trace_path.relative_to(SKILL_DIR).as_posix()}"]
    if completed.returncode == 0:
        checks.append("codex exec exit_code=0")
    else:
        checks.append(f"codex exec exit_code={completed.returncode}")
