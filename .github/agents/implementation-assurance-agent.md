...
3. **Execute & validate:** apply minimal changes, run required commands, and capture outputs. If local machine capacity, restore, or MSBuild locks block proof, push the branch and use GitHub Actions `Targeted Test` with the same repo-relative .NET test project under `tests/` plus filter, or dashboard test file, before retrying broad local scripts. For performance-sensitive paths, explicitly address allocation and async blocking risks.
