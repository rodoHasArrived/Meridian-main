...
| Desktop Installer Packaging | `desktop-installer-packaging.yml` | Tag pushes (`v*`), manual | Runs a WPF release preflight, builds x64/ARM64 MSIX and consumer setup artifacts, signs tag releases, generates checksums/SPDX SBOM/GitHub attestations, and blocks release publication on native x64 and ARM64 N-1 install/launch/update/repair/rollback/uninstall receipts. The ARM64 certification job runs on the GitHub-hosted `windows-11-arm` label. A non-tag dispatch with `certify_lifecycle` proves the installed lifecycle against a throwaway self-signed certificate; only a tag run with the protected signing secret produces release evidence. `allow_first_release` certifies a release with no published predecessor, recording the update and rollback legs as not-applicable. | Signed installers, checksum/SBOM/provenance, release manifests, and clean-machine lifecycle receipts |
