...
| Web Screenshot Capture | `web-screenshot-capture.yml` | Manual only | Captures browser workstation screenshots from the configured route list with `npm install --include=optional`, cached Playwright Chromium setup, uploads artifacts, and opens a `peter-evans/create-pull-request` PR (`automation/web-screenshot-capture`) with the refreshed catalog. It never pushes commits directly. | Web screenshot artifacts; screenshot refresh PR |
