...
The default run builds `src/Meridian.Wpf/Meridian.Wpf.csproj`, builds `tests/Meridian.Wpf.Tests/Meridian.Wpf.Tests.csproj`, and runs the focused `DesktopWorkflowScriptTests` slice with `--no-build`. When `-NoIsolation` is used, the build phase also uses `--no-dependencies` after restore so this lane proves the WPF shell and WPF test project without rebuilding shared dependency projects that already have their own proof lanes. It writes logs plus JSON and Markdown summaries under `artifacts/wpf-validation/dev-loop/<timestamp>/`, uses the existing restored `obj/` graph by default to match the no-restore inner-loop command, and retries once after stopping only stale repo-owned `testhost.exe` processes if a build step fails while one is still running. The wrapper also fails fast when another repo-owned build, test, restore, MSBuild, `csc`, or `VBCSCompiler` worker is active, because overlapping WPF builds commonly produce misleading shared-output failures; unrelated `dotnet run` commands are not treated as WPF output contention. Pass `-AllowConcurrentDotnet` only when overlap is intentional. Use `-Restore` when packages, generated assets, or intermediate output changed; that path restores first and uses isolated output under `artifacts/bin/<wpf-dev-test-*>` and `artifacts/obj/<wpf-dev-test-*>` unless `-NoIsolation` is also supplied.
