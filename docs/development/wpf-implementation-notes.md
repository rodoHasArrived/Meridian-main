...
**Order Book order-flow posture** — `OrderBookPage` now places a compact posture strip between the page hero and depth ladder. `OrderBookViewModel` projects selected-symbol scope, bid/ask row counts, spread, cumulative delta, tape readiness, and bid/ask pressure into one next handoff using the already-loaded ladder and recent-trade window, without adding another polling path or changing the allocation-sensitive heatmap render loop.
