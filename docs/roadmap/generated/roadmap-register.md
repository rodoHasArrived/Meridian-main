...
Rank 4 of the 2026-07 W10 depth slate and predominantly activation rather than construction. Bulk casework is already implemented end to end with an idempotency key, a bounded case count, dry-run and partial-success handling, retained receipts, a mapped endpoint, and three browser client functions, yet no screen calls any of them and the queue remains single-select. Separately, the statement break classifier is constructed inline rather than injected, so its materiality policy is never configurable, and its result reaches the case only as a create-or-drop decision, a severity, a rationale, and interpolated prose. The canonical break type and recommended action survive only inside formatted message text, and materiality and absolute variance do not survive at all, so the natural clustering key is not queryable on a stored break. This row persists the classification as structured fields, groups a run into clusters, and wires the existing bulk rails into the queue.
