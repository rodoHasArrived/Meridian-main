...
- The queue shows what is new, what remains open and for how long, and what cleared since the prior run, without hiding open work behind a default filter. A break reads as cleared only when the successor run reconciled successfully over the same account, source, period, and both the mapping and tolerance profile at a known matching version - a successful run that still carries open breaks counts, a failed, validation-failed, or cancelled one does not, and an unrecorded profile version is treated as unknown rather than as agreement; after an inconclusive or scope-divergent run the prior break stays open rather than disappearing.
...
- An operator comparing cost-basis relief methods for a pending disposal sees realized gain, character split, and wash-sale exposure per method, and any figure the engine cannot yet compute completely renders as incomplete rather than as a settled zero. A disposal that could still attract a wash sale while its replacement window is open - one a wash-sale policy actually governs on its own sale date, and that either realizes a loss or holds mixed gain and loss lots meaning a loss cannot yet be excluded - is labelled provisional with the remaining window shown and is re-evaluated or governed-finalized once that window closes. A disposal no governing policy reaches, or whose every relieved lot realizes a gain, reads as settled rather than being marked provisional for a window that cannot change it.
