...
AI regressions should be recorded here when they are found and fixed:
...
3. Add or update an entry in this file during the fix, or use the local docs automation scripts if
   an intake helper is reintroduced.

The former GitHub Actions intake job is archived with the legacy workflow set. Do not assume a
label alone will update this file.
...
### AI-20260916-phase-scope-check-passes-vacuously-on-staged-changes
- **ID**: AI-20260916-phase-scope-check-passes-vacuously-on-staged-changes
- **Area**: ci/governance
- **Symptoms**: `python3 tools/roadmap/enforce_phase_scope.py --phase PRx` prints `Phase scope gate passed for PRx` locally, and the hosted `scope-gate` then fails on the same change with `--- violating-files` naming paths the phase does not allow. The local run and the CI run disagree completely, on the same working tree.
- **Root cause**: `get_changed_files` runs `git diff --name-only <base>...<head>` (`enforce_phase_scope.py:105-106`) with `--head-ref` defaulting to `GITHUB_SHA` or `HEAD` (`:164`). A **staged but uncommitted** change is in the index, not in `HEAD`, so the three-dot diff sees none of it: the gate evaluates an empty file list and passes vacuously. Running the check after `git add -A` but before `git commit` — the natural moment, right when you are assembling the change — is exactly when it is blind. The same applies to unstaged edits.
- **Prevention checklist**: (1) Run the gate **after committing**, never between `git add` and `git commit`. (2) Pass the refs explicitly — `--base-ref origin/main --head-ref HEAD` — and `git fetch origin main` first, so the base is not a stale local ref. (3) Sanity-check that the run is not vacuous: a passing run on a real change prints an allowlist; if you cannot see the change reflected, assume it saw nothing. (4) Remember the allowlists are **cumulative and path-prefixed** (`:26-41`): `PR7` is `PR6 + src/**`, so a top-level directory such as `Meridian Design System/` is covered by **no tier below `PR9` (`**`)** — vendored packages outside `docs/`, `src/`, `build/`, `tests/`, `scripts/`, `schemas/` and `.github/` all fall in that gap.
- **Source issue**: PR #2977
- **Status**: mitigated
- **Fixed in**: Documented here. The failure mode is a verification that answers a question about `HEAD` while the author is asking about the working tree — the same shape as `AI-20260828-pr-body-rewrite-drops-phase-marker`, where the check read a sanitized or stale copy of the thing it claimed to be checking. When a local gate and its hosted counterpart disagree, suspect the inputs differ before suspecting the logic does.
