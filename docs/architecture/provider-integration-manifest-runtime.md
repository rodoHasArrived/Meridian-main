...
| 4. Operator setup | Expose guided setup and review through shared workstation endpoints. | The application monitoring service now composes WPF/browser-compatible connection read models from durable sync-run, staging, quarantine, and validation evidence; endpoint and screen wiring still need to show draft state, tests, activation blockers, sync runs, and quarantine groups. |
