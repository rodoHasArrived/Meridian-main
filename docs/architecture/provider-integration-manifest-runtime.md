...
quarantine records, schema snapshots, and replay evidence are operational records and need a
storage-owned boundary with WAL or `AtomicFileWriter` durability.
...
| 2. Dry-run runner | Execute manual CSV upload and one REST pull-mode endpoint without loading canonical stores. | Raw payload retention, record extraction, cursor pagination, and dry-run summary work for manual CSV and one custodian-position sample provider. |
...
| 4. Operator setup | Expose guided setup and review through shared workstation endpoints. | Browser setup flow and WPF-compatible read models show draft state, tests, dry-run evidence, activation blockers, sync runs, and quarantine groups. |
...
1. Contracts and JSON context:
   Add manifest, endpoint, mapping, validation, sync, quarantine, and activation-readiness DTOs.
2. Manifest store:
   Add `Meridian.Storage.Integrations` with local durable stores for provider templates, connection
   instances, manifest versions, raw payloads, quarantine records, integration staging records, and
   activation evidence.
3. Runtime skeleton:
...
5. UI services and endpoints:
   Add shared endpoints for catalog, connection draft, test auth, test endpoint, sample preview,
   mapping preview, dry run, activation readiness, activation, sync runs, and quarantine.
...
- Confirm `src/Meridian.Storage/Integrations/` as the first durable storage owner for manifests, raw
  payloads, quarantine records, staging records, and replay evidence.
