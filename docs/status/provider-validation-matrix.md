...
| Scope | Offline / CI evidence | Manual / runtime evidence | Fallback / rollback strategy | Validation status | Bounded by |
| --- | --- | --- | --- | --- |
| Alpaca core provider confidence | `AlpacaBrokerageGatewayTests`, `AlpacaCorporateActionProviderTests`, `AlpacaCredentialAndReconnectTests`, `AlpacaMessageParsingTests`, `AlpacaQuotePipelineGoldenTests`, `AlpacaQuoteRoutingTests`, `ExecutionGovernanceEndpointsTests.AlpacaExecutionPath_SubmitsOrderThroughStableExecutionSeam` | Not required for the active Wave 1 claim | Primary via Alpaca adapter; streaming fallback follows configured `FailoverRuleConfig` backup order and historical fallback follows composite/priority provider selection. Rollback is auto-recovery to primary after recovery threshold successes or manual force failover reset. | ✅ Closed | n/a |
| Robinhood supported surface | `RobinhoodBrokerageGatewayTests`, `RobinhoodMarketDataClientTests`, `RobinhoodHistoricalDataProviderTests`, `RobinhoodSymbolSearchProviderTests`, `ExecutionGovernanceEndpointsTests.RobinhoodExecutionPath_SubmitsOrderThroughStableExecutionSeam` | Bounded broker-session scenarios (`auth-session`, `quote-polling`, `order-submit-cancel`, `throttling-reconnect`) must be regenerated or attached for the review run; the old `artifacts/provider-validation/robinhood/2026-04-09/` packet is not retained in the current repo | Primary via Robinhood polling adapter where enabled; fallback routes to configured streaming backups and historical composite alternatives (for example Yahoo historical) when Robinhood health degrades. Rollback requires stable success threshold recovery before returning to primary and may require manual operator force failover in sustained instability. | ⚠️ Bounded | Unofficial API plus manual broker-session and runtime requirements |
| Yahoo historical and fallback confidence | `YahooFinanceHistoricalDataProviderTests`, `YahooFinanceIntradayContractTests` | Not required for the active Wave 1 claim; existing live Yahoo integration suites are optional developer reference only | Yahoo is an explicit historical fallback row; selection is by composite/priority order when preferred providers are unhealthy/unavailable. Rollback is automatic when higher-priority provider health recovers and registry selection returns to normal ordering. | ✅ Closed | n/a |
| Checkpoint reliability | `BackfillStatusStoreTests`, `ParallelBackfillServiceTests`, `GapBackfillServiceTests`, `CheckpointEndpointTests` | Not required; the Wave 1 claim is closed in repo tests | Not a provider row; operational fallback is checkpoint resume and replay continuity paths that preserve backfill progress during provider degradation. Rollback is resume-to-primary provider flow after connectivity/health restoration. | ✅ Closed | n/a |
| TradeStation execution evidence reconciliation slice | `PaperSessionPersistenceServiceTests.TradeStationExecutionSlice_CreateUpdateCancelAndFillReconciliation_ProducesDeterministicCanonicalEvidence`, `PaperSessionPersistenceServiceTests.TradeStationExecutionSlice_DelayedOutOfOrderEvents_RemainsIdempotentAndDeterministic` | Not required; this row is repo-closed evidence for create/update/cancel plus delayed/out-of-order fill reconciliation determinism into canonical execution evidence | Execution evidence fallback relies on deterministic canonical replay/reconciliation capture when live provider telemetry degrades. Rollback is restoration of normal live-fed reconciliation once provider health is re-established. | ✅ Closed | n/a |
| Parquet L2 flush behavior | `ParquetStorageSinkTests`, `ParquetConversionServiceTests` | Not required; the Wave 1 claim is closed in repo tests | Not a provider selection row; fallback strategy is durable storage flush/retry and replay safety to maintain readiness evidence during transient provider outages. Rollback is normal flush cadence once ingress recovers. | ✅ Closed | n/a |
| Execution/readiness parity slice (IBKR-focused contract stability) | `IBBrokerageGatewayTests.ConnectAsync_AfterReconnect_RehydratesSessionAndAllowsOrderLifecycleToContinue`, `IBBrokerageGatewayTests.GetPositionsAsync_MapsPositionCallbacks`, `TradingOperatorReadinessServiceTests.GetAsync_AfterRestart_ShouldPreserveReplayParityAndExecutionAuditEvidence` | Use run-date replay/session artifacts only when validating with a live broker gateway; CI closes the canonical projection stability contract for auth/session refresh, position snapshots, and replay-readiness reconstruction | Primary via IBKR execution adapter contract; fallback uses replay/readiness evidence continuity and configured provider alternatives for data acquisition lanes where applicable. Rollback is reconnect/session recovery plus automatic primary restoration after sustained success. | ✅ Closed | n/a |
