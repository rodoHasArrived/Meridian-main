...
- `GET /api/workstation/data` provider rows expose `trustScore`, `signalSource`, `reasonCode`, `recommendedAction`, and `gateImpact` for provider metrics loaded from `_status/providers.json`; `/api/workstation/data-operations` remains a compatibility alias.
- The WPF Data provider queue includes the DK1 signal source, reason code, and recommended action in its visible provider-health detail when provider routing is degraded or disconnected.
