...
**Status:** Development / Pilot Ready - Wave 1 trust gate is closed; Waves 2-4 productization remain active while WPF orientation/routing support continues to tighten
...
| WPF workstation shell + retained local API/web support surfaces | Partial | The primary WPF shell exposes meaningful workspace flows and projects Trading, Trading Hours, OrderBook posture, Research, Data Operations, Provider Health, System Health, Notification Center, Activity Log, Watchlist pinned-first staging, StrategyRuns, QuantScript, Fund Accounts account and balance-evidence posture, operator queue, and Messaging Hub support surfaces from shared readiness/run/provider/depth/account/calendar/local history/messaging inputs; retained local API/web surfaces support the same contracts while workflow hardening and deeper workflow-first consolidation remain |
