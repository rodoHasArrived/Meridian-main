...
**Status:** Development / Pilot Ready - Wave 1 trust gate is closed; Waves 2-4 productization remain active while the browser workstation is the active operator UI lane and retained WPF orientation/routing plus release-evidence automation continue to support compatibility; current UFL/reference-data and browser Security Master deltas are support evidence, not readiness exits
...
| Web workstation dashboard + retained WPF/local API support surfaces | Partial | The active browser workstation now exposes overview/strategy/trading/portfolio/data/accounting/reporting/settings routes, Research run-library support, gated Quant Lab execution/parameter/template support, workflow command-palette commands, Portfolio brokerage-sync evidence, Overview portfolio-at-a-glance, richer sidebar/menu/command grouping, menu-linked screenshot routes, Reporting report-pack task state and Evidence Workbench, Settings backend capability/Alpaca connection posture, Data Security Master print-packet readiness, browser Security Master details/lots/operator overrides, live quote/order-book/watchlist workflows, multi-symbol quote snapshots, click-to-trade order-ticket staging, historical charts, seeded no-host AAPL market-data fixtures, simplified rail/header context, shared reference-data and evidence packet/graph/validation/manifest-export APIs, and hardened command-palette/mega-menu/Portfolio filter focus behavior; retained WPF surfaces project Trading, Trading Hours, OrderBook posture, Research, Data Operations, Storage archive posture, Provider Health, System Health, Notification Center, Activity Log, Watchlist pinned-first staging, StrategyRuns, QuantScript, Fund Accounts account and balance-evidence posture, operator queue, and Messaging Hub support surfaces from shared readiness/run/provider/depth/account/calendar/storage/local history/messaging inputs. Workflow hardening, Evidence Vault/report-line provenance, and deeper workflow-first consolidation remain |
...
- governance still needs deeper account/entity, multi-ledger, cash-flow, reconciliation, and governed reporting workflows; the Fund Accounts workbench now projects account-queue, provider-routing, shared-data, retained balance-evidence, and ready-for-reconciliation states from loaded evidence, browser Portfolio now surfaces brokerage household/account posture, browser Reporting now exposes report-pack task state, Security Master now has browser details/lots/operator overrides plus UFL/reference-data projection support, and the first reconciliation break-queue slice is file-backed with review, resolve/dismiss, audit-history routes, seeded exception-route/tolerance/sign-off metadata, and calibration-summary rollups, but operator-approved calibration, external-account acceptance, report-pack lifecycle acceptance, and generalized governance casework remain open
