...
- governance still needs deeper account/entity, multi-ledger, cash-flow, reconciliation, and governed reporting workflows; the Fund Accounts workbench now projects account-queue, provider-routing, shared-data, retained balance-evidence, and ready-for-reconciliation states from loaded evidence, browser Portfolio now surfaces brokerage household/account posture, browser Reporting now exposes report-pack task state, Security Master now has browser details/lots/operator overrides plus UFL/reference-data projection support, and the first reconciliation break-queue slice is file-backed with review, resolve/dismiss, audit-history routes, seeded exception-route/tolerance/sign-off metadata, calibration-summary rollups, and browser Accounting detail-queue selection/empty-state guidance, but operator-approved calibration, external-account acceptance, report-pack lifecycle acceptance, and generalized governance casework remain open
