...
The current repository reinforces that direction rather than changing it. The WPF workspace-shell baseline is present through `ShellNavigationCatalog`, workspace shell pages, `MainPageViewModel` orchestration, deep-page hosting, shell-context strips, shell smoke tests, the Trading desk briefing hero, the Research desk briefing hero, the Data Operations desk briefing hero, Provider Health posture briefing, System Health triage briefing, Notification Center history recovery, Activity Log triage strip, the main-shell operator queue button backed by `/api/workstation/operator/inbox`, Messaging Hub delivery-posture state with refresh recency, StrategyRuns filter recovery, route-aware run review-packet work items, QuantScript local run-history handoffs, Fund Accounts account-queue/provider-routing/shared-data briefing state, focused tests for Batch Backtest, Position Blotter, Fund Accounts, QuantScript, Notification Center, Welcome, System Health triage behavior, Activity Log triage behavior, workspace queue tone styles, workspace shell context-strip behavior, Trading hero state selection, Research run/promotion handoffs, Provider Health posture states, hidden-but-present workflow page-state markers, deterministic isolated workflow restore/build behavior, and local single-instance mutex plus launch-argument forwarding coverage. Workflow-level acceptance still belongs to Waves 2-4 rather than a separate desktop migration milestone.
...
| Governance product surfaces | Partial | Security coverage, Fund Accounts account/routing posture, reconciliation drill-ins, direct lending, and reporting-adjacent seams are live; broader multi-ledger, cash-flow, durable casework, and governed reporting workflows remain incomplete |
| WPF workstation shell + retained local API/web support surfaces | Partial | The primary WPF shell exposes meaningful workspace flows and projects Trading, Research, Data Operations, Provider Health, System Health, Notification Center, Activity Log, Watchlist, StrategyRuns, QuantScript, Fund Accounts, operator queue, and Messaging Hub support surfaces from shared readiness/run/provider/account/local history/messaging inputs; retained local API/web surfaces support the same contracts while workflow hardening and deeper workflow-first consolidation remain |
...
- primary WPF workstation shell plus retained local API/web support surfaces that consume shared workstation contracts
...
- workflow guide, screenshot-refresh tooling, Provider Health and System Health posture cues, page-state automation markers, and single-instance launch forwarding for ongoing operator-facing validation
...
- governance still needs deeper account/entity, multi-ledger, cash-flow, reconciliation, and governed reporting workflows; the Fund Accounts workbench now projects account-queue, provider-routing, shared-data, and ready-for-reconciliation states from loaded evidence, and the first reconciliation break-queue slice is file-backed with review, resolve/dismiss, and audit-history routes, but tolerance calibration, external-account acceptance, and generalized governance casework remain open
