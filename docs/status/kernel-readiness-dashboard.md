...
- **Current sync note (2026-05-29):** Wave 1 provider evidence posture remains the packet-bound 2026-04-27 artifact set until a newly generated packet and matching sign-off replace it. Wave 2 paper-cockpit acceptance is now a closed baseline per [`PROGRAM_STATE.md`](PROGRAM_STATE.md) and the latest `artifacts/pilot-acceptance/latest/pilot-readiness.json` W2 gates; DK2 export and reconciliation governance remain active work.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Data quality + provider trust | DK1 | Data Operations & Provider Reliability owner | ✅ | ✅ | ✅ | ✅ | Ready | Keep Wave 1/DK1 evidence synchronized during Wave 2 cockpit handoff (2026-05-01) | Evidence pack: [DK1 pilot parity runbook](./evidence/dk1-pilot-parity-runbook.md), [trust rationale mapping](./evidence/dk1-trust-rationale-mapping.md), [baseline thresholds + FP/FN review](./evidence/dk1-baseline-trust-thresholds.md), [provider validation matrix](./provider-validation-matrix.md), and the 2026-04-27 run artifacts at `artifacts/provider-validation/_automation/2026-04-27/wave1-validation-summary.json`, `.md`, `dk1-pilot-parity-packet.json`, and `.md`. The fresh DK1 packet reports `ready-for-operator-review`, all four pilot sample rows `ready`, all four evidence documents `validated`, and validated explainability/calibration contracts. The packet-bound sign-off file at `artifacts/provider-validation/_automation/2026-04-27/dk1-operator-signoff.json` is signed by `RODO` for Data Operations, Provider Reliability, and Trading; validation reports packet binding `valid`, `operatorSignoff.status=signed`, `operatorSignoff.validForDk1Exit=true`, and no missing owners. The removed `codex-dk1-packet-validation-final` path must not be treated as current evidence; future DK1 packets must be fresh date-stamped outputs with matching packet-bound sign-off before replacing the 2026-04-27 evidence. |
| Promotion + paper-trading cockpit | DK1 -> DK2 handoff | Trading Workstation owner | ✅ | ✅ | ✅ | ✅ | Ready | Preserve Wave 2 cockpit evidence while DK2 export/reconciliation gates advance (2026-05-29) | Wave 2 cockpit acceptance is closed as a baseline in [`PROGRAM_STATE.md`](PROGRAM_STATE.md): five named operator-state tests, work-item action routing for PaperReplay/ExecutionControl/PromotionReview/SecurityMasterCoverage/ReconciliationBreak, stale-replay recovery cockpit workflow test, approval checklist field + validation (auto-populated from evaluation, clears on runId change), fund-account context threading to `getTradingReadiness`, session persistence cockpit test (create→verify→close), and green cockpit proof (`ReadyForPaperOperation=true` only with DK1 signed + healthy brokerage + active session + consistent replay). Evidence: [wave2-cockpit-evidence-packet.md](./evidence/wave2-cockpit-evidence-packet.md) and `artifacts/pilot-acceptance/latest/pilot-readiness.json` stages `TrustedData`, `PaperPromotion`, and `PaperSession`, all `Ready` with no blockers. DK2 export and reconciliation governance remain open in their own rows rather than reopening W2. |
...
| 2026-05-16 -> 2026-05-29 | Reconciliation + governance (DK2) | Extend the file-backed reconciliation break queue into operator-approved tolerance profiles, exception severity routing, and governance sign-off evidence | Current acceptance artifacts: `WorkstationEndpointsTests.MapWorkstationEndpoints_BreakQueueRoute_ShouldHydrateQueueWithoutGovernanceBootstrap`, `MapWorkstationEndpoints_ReconciliationCalibrationSummary_ShouldAggregateToleranceProfiles`, `MapWorkstationEndpoints_ReconciliationCalibrationSummary_AfterResolution_ShouldMarkReadyForSignoff`, `MapWorkstationEndpoints_BreakQueueReviewRoute_ShouldHydrateQueueWithoutListBootstrap`, and `MapWorkstationEndpoints_BreakQueueResolveRoute_ShouldRequireReviewBeforeResolve`; `/api/workstation/reconciliation/calibration-summary` now summarizes tolerance profiles, active critical breaks, pending sign-off, missing metadata, and sign-off readiness while exception burn-down remains pending |
...
- [x] **Parity entry:** Wave 1 closure evidence remains current and reproducible ([DK1 pilot parity runbook](./evidence/dk1-pilot-parity-runbook.md)).
- [x] **Explainability entry:** trust signals have operator-visible source and rationale ([trust rationale mapping](./evidence/dk1-trust-rationale-mapping.md)).
- [x] **Calibration entry:** baseline trust thresholds are declared for pilot operations ([baseline thresholds + FP/FN review](./evidence/dk1-baseline-trust-thresholds.md)).
- [x] **Operator entry:** Data Ops + Trading approve DK1 pilot scope through the packet-bound 2026-04-27 sign-off file.

### Exit checklist

- [x] **Parity pass:** cockpit views match validated provider/replay results for pilot scenarios ([DK1 pilot parity runbook](./evidence/dk1-pilot-parity-runbook.md)).
- [x] **Explainability pass:** every trust alert maps to source, reason code, and operator action ([trust rationale mapping](./evidence/dk1-trust-rationale-mapping.md)).
- [x] **Calibration pass:** thresholds tuned with documented false-positive/false-negative review ([baseline thresholds + FP/FN review](./evidence/dk1-baseline-trust-thresholds.md)).
