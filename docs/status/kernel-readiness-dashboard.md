...
| Shared run/portfolio/ledger interop contracts | Cross-wave (DK1 + DK2) | Shared Platform Interop owner (Architecture + Contracts) | 🟡 | 🟡 | 🟡 | ⚪ | At Risk | Attach first generated contract review packet to the weekly interop review and record owner decision (2026-04-29) | Evidence: [contract compatibility matrix](./contract-compatibility-matrix.md), `scripts/check_contract_compatibility_gate.py`, and `scripts/generate_contract_review_packet.py`. The gate now covers public removals, shared `UiApiRoutes` route constant removals/value changes, scoped record constructor parameter removals, and enum-member removals; the packet generator makes tracked-surface review and migration-note status repeatable. Owner sign-off on the review cadence remains open. |
