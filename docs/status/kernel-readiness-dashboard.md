...
| Promotion + paper-trading cockpit | DK1 -> DK2 handoff | Trading Workstation owner | 🟡 | 🟡 | ⚪ | ⚪ | In Progress (pending operator acceptance) | Operator acceptance sign-off on Wave 2 cockpit evidence packet (2026-05-28) | Wave 2 cockpit completion delivered: five named operator-state tests, work-item action routing for PaperReplay/ExecutionControl/PromotionReview/SecurityMasterCoverage/ReconciliationBreak, stale-replay recovery cockpit workflow test, approval checklist field + validation (auto-populated from evaluation, clears on runId change), fund-account context threading to `getTradingReadiness`, session persistence cockpit test (create→verify→close), and green cockpit proof (`ReadyForPaperOperation=true` only with DK1 signed + healthy brokerage + active session + consistent replay). Evidence: [wave2-cockpit-evidence-packet.md](./evidence/wave2-cockpit-evidence-packet.md). Pilot audit sample: `ExecutionWriteEndpointsTests.Scenario_SessionCloseReplayAndPromotionReview_BacktestToPaperFlowRemainsContinuousAndAuditable`; inbox routing: `WorkstationEndpointsTests.MapWorkstationEndpoints_OperatorInbox_ShouldIncludeRunReviewPacketWorkItems`. DK2 entry/exit gates remain open pending operator acceptance sign-off. |
