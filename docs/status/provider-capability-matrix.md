...
| `InteractiveBrokers` | complete | complete | template-only | template-only | template-only | complete | complete | Brokerage Integrations | IB Gateway/TWS runtime dependency, IBAPI version compatibility, session lifecycle and entitlement complexity. |
...
| `Robinhood` | complete | complete | complete | complete | template-only | partial | partial | Brokerage Integrations | Unofficial/consumer workflow volatility, auth/token lifecycle sensitivity, non-deterministic brokerage behavior. |
