...
Master resolution and override approval, ledger draft/validate/post, reconciliation, approval,
...
2026-05-21 documentation and strategy-control note: Strategy Engine definitions/validate-run
...
- the feature inventory and implementation anchors show that the active browser and WPF workstations now has a Research run library with retained-run review, two-run compare/diff readiness, promotion-history loading, command-error alerts, focused tests, and refreshed built workstation assets. Retained WPF consolidation, the Trading desk briefing hero with stale-replay count detail, Position Blotter selection review/action readiness, Trading Hours live-risk/pre-market/after-hours/closed-planning briefing states, OrderBook order-flow posture, the Research desk briefing hero, Data Operations briefing state, Storage archive growth/capacity/last-scan posture, Provider Health posture briefing, System Health triage, Notification Center filter recovery, Activity Log triage with export/clear command state, Watchlist posture plus pinned-first staging, Messaging Hub delivery posture with refresh recency, StrategyRuns filter-aware recovery and run-scope presentation, BatchBacktest results empty guidance, QuantScript local execution-history presentation with Research handoffs for mirrored runs, stable route-aware run review-packet work items, Security Master runtime/search recovery, Fund Accounts account-queue/provider-routing/shared-data and balance-evidence briefing, shared workstation endpoints, canonical `ResearchShell` launch/deep-link handling, fixture/demo-mode cues, UI-automation page-state markers, corrected isolated desktop restore/build behavior, local single-instance mutex and launch-argument forwarding coverage, the Wave 2 trading-readiness contract with stable operator work-item IDs, the initial operator-inbox endpoint for readiness, actionable latest-run review-packet, and reconciliation work items, route-aware WPF shell queue-button consumption of the primary operator work item with active-account `fundAccountId` propagation and shell-context attention cues including brokerage-sync routes into Account Portfolio, risk/control audit explainability projection, DK1 trust-gate readiness projection, canonical promotion approval-checklist state, and the Security Master baseline are materially present in the repo, but still not enough to call the end-to-end product finished
- the newer browser workstation evidence replaces more placeholder behavior with shared workflow commands, Portfolio brokerage household/account review with account-filter focus restoration, Portfolio position and run-evidence dense tables that drive accessible detail panels, Reporting report-pack task panels and the `/reporting/evidence` workbench, Settings backend capability coverage plus Alpaca paper-key verification, Data Security Master print-packet readiness, Data backfill dense-table queue selection with keyboard-expanded detail, browser Accounting reconciliation dense-table selection, keyboard-expanded detail, no-host break-queue fixture, and empty-state guidance, live quote/order-book/watchlist workflows backed by multi-symbol quote snapshots and selectable recent-trade detail inspection, Price Alerts, click-to-trade order-ticket staging, intraday and historical price charts, a gated `/strategy/quant-lab` surface backed by `/api/quant/run`, `/api/quant/parameters`, and `/api/quant/templates`, a visual `/strategy/designer` surface, Overview portfolio-at-a-glance plus Today panel, sidebar sub-items plus richer menu/command grouping, menu-linked screenshot-route coverage, full-console readiness checkpoints, browser Security Master search-result selection, identity drill-ins, details/lots tracking, server-side Security Master operator overrides, no-host seeded quote/trade/depth/symbol/snapshot/historical-bar fixtures for `/data/quotes?symbol=AAPL`, Meridian Design System reference workbench/tokenized-color support, and simplified browser rail/header chrome that avoids repeated current-workspace context; shared services also now expose fund-account brokerage links, positions/activity, performance, cash-flow, household portfolio rollups, a read-only Robinhood aggregation adapter, UFL/reference-data projections for bonds, options, equities, futures, FX spot, swaps, commodities, crypto, deposits, money-market funds, and certificates of deposit, and evidence subject/packet/graph/validation/manifest-export APIs. These raise the value of Wave 2/Wave 3/Wave 4 continuity work and buyer-demo packaging, but they remain support evidence until cockpit, durable reconciliation casework, report-pack, Evidence Vault, report-line provenance, and live-readiness gates pass
- the current support hardening adds provider-setup next-validation actions for live quotes/backfill/readiness/Security Master coverage, grouped command-palette sections with hash-aware `/settings#alpaca-provider-setup` matching, Portfolio brokerage-sync next actions into provider repair, Trading readiness, and cockpit review, Watchlist no-quote/empty-value/busy copy owned by the view model, Overview status/refresh view-model presentation plus Today panel context, Data backfill dense-table queue selection, local Price Alerts, Reporting report-pack preview/export actions plus endpoint-link safety for GET versus POST routes, Security Master search-result drill-in, lot-detail selection plus conflict refresh/retry state, Trading readiness UTC labels plus loading-state panel and selectable Recent Fills detail support, full-console readiness checkpoint gates with provider-setup repair handoffs for BrokerageSync blockers, Live Quotes recent-trade detail state plus quick-trade readiness handoffs after accepted/rejected submissions, visual Strategy Designer, Covered Call chain preview, saved-run history, and selectable trade-timeline detail state, Quant Lab plot geometry and accessibility state in a reusable view model, Quant Notebook cell/data-fetch helpers, browser Accounting dense-table detail-queue state with keyboard selection and no-host break-queue fixture coverage for loaded reconciliation runs, crash-safe shared UI persistence for config/export-preset/quality-archive JSON, Meridian Design System reference workbench/tokenized-color support, and a WPF Data shell feature module. These reduce operator dead ends and implementation coupling, but they remain support evidence rather than new completion claims
...
| ---: | --- | --- | --- | --- |
| 1 | Run Evidence Graph | Very high | Partial support now exists through shared evidence subject/packet/graph APIs and browser Evidence Workbench actions, plus provider packets, shared run continuity, promotion checklist state, paper replay metadata, reconciliation work items, and report-pack references. The canonical cross-lifecycle graph remains open | Wave 3 backbone |
...
| 3 | Reconciliation Casework Module | Very high | Early in progress through file-backed reconciliation break queues, seeded owner/tolerance/sign-off metadata, audit history, calibration-summary rollups, browser Accounting dense-table detail-queue selection, keyboard-expanded detail, no-host break-queue fixture coverage, and operations-continuity break resolution in the account-period close lane | Wave 4 |
...
| 5 | Operator Readiness Console | High | Partial browser support evidence through the read-only `/trading/readiness` console, full-console checkpoint gates, shared workstation readiness payloads, workflow command-palette commands, and Data market-context routes for live quotes, order book, recent-trade detail, watchlists, Price Alerts, and historical charts | Wave 2 support surface |
...
| 9 | Fund Close Checklist | High | Partial support now exists through the operations-continuity close-lane API with explicit broker, Security Master, ledger, reconciliation, approval, close, and reopen gates; browser workflow acceptance, external statement/custodian intake, close package publication, and SLA/casework depth remain open | Wave 4 later slice |
...
| 11 | Portfolio Impact Simulator | Medium-high | Planned; portfolio/ledger/account posture seams now include read-only brokerage household rollups and performance/cash-flow support, but pre-approval impact simulation remains open | After Wave 3 continuity |
...
| 2 | Close the Books Workflow | Turns fund operations into a recurring month-end or quarter-end workflow with task status, owner, blocker, evidence, sign-off, and close package | Partial support through the shared operations-continuity API and hash-chained workflow timeline; operator-owned browser close workflow, external statement/custodian intake, publication controls, and casework depth remain open | Wave 4 later slice |
...
| 6 | Reconciliation Casework Module | Turns breaks into owned operational cases with severity, aging, tolerance, evidence, comments, decisions, and sign-off | Early in progress through file-backed break queues, metadata, audit history, calibration rollups, and close-lane break resolution; generalized durable casework remains open | Wave 4 |
| 7 | Evidence Completeness Score | Makes readiness visible across runs, promotions, paper sessions, reconciliation cases, and report-pack previews | Partial support through trading readiness summaries and Evidence Workbench packet validation; close/report-pack/reconciliation scoring is still open | Wave 3 into Wave 4 |
...
| 10 | Buyer Demo Mode | Makes the browser dashboard commercially demonstrable with seeded funds, accounts, strategies, paper sessions, breaks, approvals, and report packs | Partial support through the demo-data banner, sample-flow links, Overview Today panel, Price Alerts, Strategy Designer, Covered Call no-host chain/run-history fixtures, and seeded AAPL market-data fixtures; buyer demo packs and role-based views remain open | Web dashboard support track |
...
- **Dependency it unlocks:** Cleaner scope control across provider/broker work, web dashboard priorities, WPF surface decisions, generated readiness dashboards, evidence graph modeling, reconciliation casework, and report-pack productization.
...
- **Current support evidence:** Browser Portfolio and shared account-sync services now expose fund-account brokerage links, household account/position rollups, performance, cash-flow summaries, Alpaca paper connection posture, and read-only Robinhood aggregation. This helps connect portfolio/account review to the shared model, but it does not close reconciliation acceptance or live-readiness.
...
- **Gap:** Security Master is already the authoritative instrument-definition seam, and governance-facing APIs and workstation surfaces exist. The WPF browser now has runtime-unavailable search recovery plus a bound `Clear Search` action, the browser Data lane derives Security Master print-packet readiness, Fund Accounts now projects account-queue, provider-routing, shared-data, retained balance-evidence, and reconciliation-readiness states from loaded evidence, browser Reporting now shows report-pack task/profile state, browser Accounting can select loaded reconciliation runs through a view-model-owned detail queue, and route-aware operator-inbox handling can open Security Master or reconciliation workbenches directly from shared work items. The first governed report-pack artifact path and file-backed reconciliation break queue are now in code with seeded exception route, tolerance profile, tolerance band, required sign-off role, sign-off status metadata, and calibration-summary profile rollups. The operations-continuity workflow now gives account-period close work a shared API for broker intake, Security Master resolution, ledger posting, reconciliation, approvals, close, and reopen, but the next real product layer still needs browser/operator acceptance, external statement/custodian intake, deeper account/entity structure, multi-ledger, cash-flow, operator-approved reconciliation calibration/casework, publication controls, and board/investor/compliance reporting workflows.
...
### 4. Preserve the signed DK1 pilot parity packet on top of the closed Wave 1 trust gate

- **Category:** provider readiness
- **Gap:** The active Wave 1 gate is now repo-closed, and DK1 has a concrete signed pilot replay/sample-set plus generated parity-packet evidence. The 2026-04-27 packet is valid for DK1 exit, but the open work is to keep the matrix, roadmap, generated runtime outputs, runbook, validation script, parity packet, trust rationale mapping, threshold calibration, and future packet-bound sign-off aligned around Alpaca, Robinhood, Yahoo, checkpoint reliability, and Parquet L2 flush proof whenever evidence changes.
...
- **Gap:** QuantScript now has a delivered local Run History tab and Research handoff path for mirrored runs, but broader QuantScript expansion, queue-aware simulation, multi-instance coordination, and later performance work still do not close the current product-readiness gap.
