...
**Status:** Repo-grounded opportunity scan aligned to the 2026-04-13 canonical roadmap refresh and the 2026-04-16 feature inventory snapshot
...
The current planning set is also more mature than the prior April 8 scan. As of 2026-04-13 and 2026-04-16:
...
That means the opportunity order should stay delivery-aware: finish trust, finish the operator lane, unify the shared model, then deepen governance on top of the delivered baseline.

---

## Top Opportunities

### 1. Close Wave 1 provider-confidence and checkpoint evidence gaps

- **Category:** provider readiness
- **Gap:** Meridian has broad provider and backfill coverage, but readiness still varies across Polygon, Interactive Brokers, Robinhood, NYSE, and StockSharp. Checkpoint and Parquet L2 reliability now have stronger repo-backed proof, but the remaining bounded scenarios still need synchronized runtime evidence, validation-matrix updates, and operator-safe wording.
- **User or operator value:** Research, replay, paper trading, and later live-readiness claims become credible only when the supported provider paths are backed by explicit evidence instead of implication.
- **Dependency it unlocks:** This is the gate for every downstream workflow claim in Waves 2-4.
- **Placement:** Critical path, Wave 1.

### 2. Harden the existing paper-trading cockpit into a dependable operator lane
...
### 3. Make the shared run / portfolio / ledger model feel like one cross-workspace product
...
### 4. Productize governance and fund operations on top of the delivered Security Master baseline
...
That is why Wave 1 trust closure still outranks additional surface expansion, and why the WPF shell work should be treated as support for the core product path rather than as a competing initiative.

---

## Recommended Next Steps

1. Keep Wave 1 provider-confidence closure as the first release gate, with the validation matrix, evidence artifacts, and repo tests treated as the source of truth.
...
5. Keep Wave 5, Wave 6, and optional advanced tracks explicitly deferred until the Wave 1-4 operator-readiness path is materially closed.
