...
| `src/Meridian.Backtesting/Metrics/BacktestMetricsEngine.cs` | 319 | `NOTE` | ❌ | /// NOTE: This is an independent computation over fill events for metric attribution purposes. |
| `src/Meridian.Backtesting/Portfolio/SimulatedPortfolio.cs` | 995 | `NOTE` | ❌ | /// NOTE: This must stay consistent with <c>BacktestMetricsEngine.ComputeRealisedPnl</c>, |
...
| `src/Meridian.Ui/dashboard/src/screens/accounting-screen.view-model.ts` | 6063 | `NOTE` | ❌ | note: event.sourceReason ?? (event.isCurrentProjection ? "Current schedule projection." : null) |
