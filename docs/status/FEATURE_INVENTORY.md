...
| Buyer Demo Mode / Role-Based Demo Views | 🔄 | Browser-first demo packaging has support evidence through the `Demo data` banner, sample-flow links into watchlist, seeded `AAPL` live quotes, readiness, and Alpaca paper setup, plus no-host quote/trade/depth/symbol/snapshot/historical-bar fixtures that populate `/data/quotes?symbol=AAPL`. Seeded funds, accounts, strategies, paper sessions, breaks, approvals, evidence packets, and role-specific CIO/trader/controller/operations/compliance/auditor views remain open. |
