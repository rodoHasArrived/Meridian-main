...
| Feature | Status | Notes |
| --------- | -------- | ------- |
| Web workstation dashboard | ⚠️ | React/Vite source in `src/Meridian.Ui/dashboard/` with built workstation assets in `src/Meridian.Ui/wwwroot/workstation/`; current screens cover overview, strategy, trading, portfolio, data, accounting, reporting, settings, and retained legacy paths, and `buildAppShellViewState` centralizes active workspace resolution plus loading, partial-degradation, retry, and bootstrap-failure status panels. Canonical seven-workspace navigation is present, the command palette now consumes shared workflow library/preset payloads, global workstation refreshes suppress stale bootstrap responses, Data includes live quotes with stale-response suppression plus single/bulk symbol watchlist management with live-price coverage, stale warnings, and manual price refresh, and placeholder routes are being replaced by dedicated task panels, while full cockpit/governance acceptance remains open |
