...
| WPF run browser/detail/portfolio/ledger surfaces | In progress | Code present in `src/Meridian.Wpf/`; StrategyRuns now has visible-versus-recorded run scope and filter-aware empty-state recovery, and BatchBacktest now has stateful results empty guidance for idle, validation-blocked, running, failed, cancelled, and populated sweep states, while broader paper/live history continuity remains open |
...
- **Fund-account operator briefing:** WPF now projects fund context, account queues, provider route previews, blocked routes, shared-data access gaps, and ready-for-reconciliation state from loaded account/provider evidence; durable account/entity casework and external-account acceptance remain Wave 4 work.
