...
| Strategy / Promotion Passport | 🔄 | Promotion checklist and review-packet support exist, and the browser Strategy workspace now includes a visual Strategy Designer support surface for multi-leg payoff/participation exploration plus Covered Call chain-preview, saved-run-history, and selectable trade-timeline detail support evidence. A canonical passport projection with owner, approved use, required evidence, open exceptions, and approval history remains planned. |
