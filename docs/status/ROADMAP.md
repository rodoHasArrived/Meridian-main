...
- preserve the closed Wave 2 cockpit reliability evidence baseline, with desktop acceptance tracked through `docs/plans/desktop-ui-workflow-acceptance-matrix.md`
- preserve the closed Wave 3 continuity baseline as one run -> portfolio -> ledger -> reconciliation workflow across high-traffic operator pages
...
The documentation rule attached to this positioning is explicit: prior plans, UI notes, and status snapshots that are deprecated by the evidence-backed investment-operations direction should move to `archive/docs/` and be referenced only as historical context. Active roadmap docs should point to the current evidence plan, the closed W2/W3 baselines, and the active W4 implementation anchors.
...
These changes improve developer/release hygiene but do not reopen or alter the closed W2 cockpit
and W3 continuity baselines, and they do not close W4 governance or live-readiness gates.
...
workflow orchestration. `FundOpsCloseLaneScenarioTests` now makes the Wave 4 close-workflow
acceptance lane explicit at the service level with happy-path close, blocker/recovery around
reconciliation and report-pack readiness, and rejected-approval recovery through ledger remediation
back to close. This remains below a Wave 4 exit until browser/operator acceptance,
...
2026-05-28 governance follow-up: newer repo evidence moves the active work from W2/W3 closure
maintenance into W4 acceptance hardening. The shared W4 endpoint and workflow pass has green
service/browser/source-route evidence for operations-continuity, reconciliation casework,
fund-operations workspace reads, report-pack workflow, Security Master endpoints,
fund-structure report-pack/cash-flow endpoints, `FundOpsCloseLaneScenarioTests`, and
`PilotAcceptanceHarnessTests`; browser W4 parity also has green operations-continuity,
governance, and reporting view-model/component tests. Additional support slices add explicit
...
The shared-run support evidence also includes `RunCashFlowPage` empty-state guidance for selected-run, missing-run, no-event, and loaded cash-flow summaries. This is part of the closed W3 continuity baseline; broader governance cash-flow/projection work remains listed under Wave 4.
...
- The Wave 2 cockpit has a shared readiness seam through `/api/workstation/trading/readiness` and `TradingOperatorReadinessDto`, joining paper-session state, replay verification, execution controls, DK1 trust-gate packet posture, promotion status, brokerage sync, acceptance gates, overall readiness, work items, and warnings into one operator-facing contract. The shared `/api/workstation/operator/inbox` endpoint aggregates those readiness work items with actionable warning/critical run review-packet items from the latest runs, open/in-review reconciliation breaks, and navigation targets, and the WPF shell queue button consumes that endpoint to show review counts, severity tone, account-scoped brokerage/readiness blockers when an account operating context is active, route-aware primary-item navigation into concrete workbenches such as `FundReconciliation` and run review packets, and shell-context attention when a review item is active. Promotion approvals also have a canonical `PromotionApprovalChecklist` for DK1 trust-packet, run-lineage, portfolio/ledger-continuity, risk-control, and live-override review requirements. This is now the closed W2 baseline; new work should keep it green rather than reopen the wave.
...
- **Wave 2:** keep the dependable paper-trading cockpit baseline green, with regression evidence tied to DK1 trust signals
- **Wave 3:** keep run history, brokerage/account posture, portfolio, ledger, cash-flow, and reconciliation on one closed cross-workspace model under the shared compatibility matrix
...
### Wave 2: Workstation paper-trading cockpit baseline

**Status:** Done as of 2026-05-27.

**Why preserve it:** Meridian already has the execution, session, promotion APIs, and acceptance evidence for the operator cockpit. Product value now depends on keeping that baseline green while W4 governance work builds on it.
...
**Preservation signal:** A strategy can move from backtest into a visible, auditable paper-trading workflow through the shared workstation contract, with the web dashboard and retained local API consuming the same readiness lane.

<a id="wave-3-shared-run--portfolio--ledger-continuity"></a>

### Wave 3: Shared run / portfolio / ledger continuity baseline

**Status:** Done as of 2026-05-27.

**Why preserve it:** The contracts and acceptance evidence now establish the shared run-centered product experience. W4 casework and reporting work should consume this baseline instead of redefining it.
...
- hand off file-backed reconciliation break queue and calibration-summary rollups to W4 for operator-approved calibration, durable casework, and external-account/custodian review flows
...
**Preservation signal:** Strategy runs remain Meridian's primary cross-workspace product object rather than one of several overlapping representations.
