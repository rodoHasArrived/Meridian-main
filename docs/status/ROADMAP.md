...
**Status:** Active productization — the narrow Wave 1 trust gate is repo-closed, DK1 is now tracked through a concrete pilot replay/sample-set and parity-packet contract, Waves 2-4 remain the core operator-readiness path, and the WPF workspace-shell baseline is in code with workflow validation still open
...
- The Wave 2 cockpit now has a shared readiness seam through `/api/workstation/trading/readiness` and `TradingOperatorReadinessDto`, joining paper-session state, replay verification, execution controls, promotion status, brokerage sync, work items, and warnings into one operator-facing contract. Promotion approvals also have a canonical `PromotionApprovalChecklist` for DK1 trust-packet, run-lineage, portfolio/ledger-continuity, risk-control, and live-override review requirements. This is useful acceptance infrastructure, not a completed cockpit claim.
...
- use the new trading-readiness contract as the acceptance surface for session state, replay consistency, audit linkage, promotion approval checklist completion, brokerage-sync posture, and operator work items
