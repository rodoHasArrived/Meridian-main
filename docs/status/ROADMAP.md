...
- The WPF shell/navigation baseline is materially delivered: four workspace shell pages, metadata-driven navigation, deep-page hosting, command/search metadata, context strips, the Trading and Research desk briefing heroes, and navigation/shell smoke tests are present. The Trading hero now projects active-run, workflow-summary, replay/readiness, controls, DK1 trust, brokerage-sync, and handoff state into the shell; the Research hero now projects market briefing, selected-run, run detail, portfolio, and promotion-review handoff state. The remaining roadmap question is still whether those surfaces measurably improve active Wave 2-4 workflows.
