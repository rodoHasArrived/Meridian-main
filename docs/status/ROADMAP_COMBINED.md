...
The latest support-surface evidence also includes Welcome readiness progress, Storage preview scope/guidance, OrderBook order-flow posture, compact shared deep-page command chrome with related-workflow/trust context, actionable shell-context attention detail, provider-degradation next actions that land on `ProviderHealth`, brokerage-sync queue routing into `AccountPortfolio`, and Trading hero handling that keeps warning or critical shared work items in an attention state before showing a ready desk. These improve orientation and handoff quality; they do not change the Wave 2-4 acceptance gates.
