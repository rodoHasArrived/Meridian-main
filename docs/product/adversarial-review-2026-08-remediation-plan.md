...
- [ ] **AR8-57 — Stop paying the post-merge docs-regeneration toll by hand.**
  ...
  **Change:** keep the artifacts tracked — `docs/documentation-ownership.md:23` designates
  ...
  survived, and they conflict with one another. Treat "design something that satisfies all six" as
  the work, not as preamble to an obvious one-liner.
  ...
  4. **The repair scope is wider than `docs/status/`.** The same `core` profile runs
     `generate-structure-docs` (`run-docs-automation.py:293-299`), which rewrites
     `docs/generated/repository-structure.md` whenever a markdown file is added, removed, or
     renamed. An allowlist covering only `docs/status/` leaves `regenerate-docs` red for exactly
     the file-topology changes that are most common in documentation work.
  5. **Generator-changing pull requests need their own path.** If constraint 1 is met by running
     trusted base-revision tooling, a PR that legitimately changes
  ...
  6. **Neither pre- nor post-merge repair is free.** Per-branch repair does not fix the merge-queue
  ...
  **Verify:** (a) merge a branch editing in-corpus markdown into a branch editing different
  ...
  running the profile; (b) do the same for two pull requests entering the merge queue together, and
  confirm the dashboard is correct on the tree that actually merges; (c) include one pull request
  that only adds or renames a markdown file, so `repository-structure.md` is exercised alongside
  the health dashboard; (d) include one that changes the generator itself. Assert the outcome — the
  generated artifacts matching the merged tree — rather than the mechanism.
  **Effort:** L, and it needs a design step before implementation. The regeneration is one command;
  the six constraints above, two of which conflict, are the actual work · **Sequence:** independent
