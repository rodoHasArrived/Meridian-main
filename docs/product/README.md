...
- If you need the current product framing, start with:
  ...
  - [2026-08-29 W9 Operator Acceptance Record](w9-operator-acceptance-2026-08-29.md) — the operator
    decision accepting six W9 rows and holding `W9-ALPACA-004`, with the evidence each was accepted
    on (decision `DEC-W9-ACCEPTANCE-001`); live status stays in the roadmap registry
...
As of 2026-08-29 the W9 slate is mostly closed: `W9-ASSET-010` is `done`; `W9-TRUTH-001`,
`W9-DEMO-002`, `W9-PAPER-003`, `W9-REPORT-005`, `W9-NAV-006`, and `W9-CORPACT-011` are `accepted`
on operator decision `DEC-W9-ACCEPTANCE-001`, recorded in the
[2026-08-29 W9 Operator Acceptance Record](w9-operator-acceptance-2026-08-29.md);
`W9-ALPACA-004` was deliberately held at that review and stays `ready_for_acceptance` pending its
three recorded fill-path caveats; and `W9-SAFETY-007`, `W9-GOV-008`, and `W9-INGEST-009` are
`in_progress`. Those three open rows are sequenced by the
