...
- `src/Meridian.Contracts/`, `src/Meridian.Ledger/`, `src/Meridian.Reporting/`, `src/Meridian.Ui.Shared/`, and `src/Meridian.Ui/dashboard/` carry the active shared source seams for those workflows. WPF remains retained compatibility and historical evidence context, but new WPF implementation, parity proof, screenshots, and validation are deferred until explicitly reactivated.
