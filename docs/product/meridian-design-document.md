...
| Statement connectors | Completed profile-driven CSV/OFX, remotely fetched or uploaded IB Flex XML, OFX bank/investment, and completely paged Alpaca connectors with shared preview, confidence, drift, idempotency, canonical account evidence, and reconciliation handoff. | Margin Control rolls provider-authoritative margin, cursor completeness, option lifecycle, tax-lot, and borrow evidence across accounts and primes while keeping Meridian shadows diagnostic and EOD certification governed. The shared connector contract supports additional institution-specific profiles without duplicating reconciliation policy. | Add provider and format depth only with retained-source, complete-cursor, drift, golden-file, and operator-acceptance evidence. |
