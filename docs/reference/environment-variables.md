...
| `MDC_ANONYMOUS_TENANT` | Runtime auth middleware | Tenant and company authority an anonymous caller carries in optional mode. Without it the `/api/workstation/*` group still refuses the caller, since a session normally takes its tenant from the account's company; the seeded demo host supplies its own tenant and needs no value here. | Names which stored records the anonymous caller reads and writes; point it at the deployment's own tenant, not a shared one. |
