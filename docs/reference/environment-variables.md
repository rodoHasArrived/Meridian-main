...
| `MDC_DISABLE_RATE_LIMIT` | Development/test rate-limiter override | `true` bypasses mutation/global API throttles only with an explicit Development/Test host and no production, packaged, or customer posture. Missing host/policy evidence keeps throttling enabled. | Never use as a production capacity control; production and customer deployments ignore the bypass request. |
