...
- 2026-07-18: Added the additive Margin Control Center DTO and route family plus optional statement-preview activity, account-margin, tax-lot, borrow, and retained-evidence fields. Existing statement import members and routes are unchanged; older browser/WPF clients may ignore the new payload members and routes. New positional DTO parameters carry safe defaults so missing fields remain forward-compatible, while the updated browser consumes the shared route family directly.
...
- 2026-07-26: Added `durableReconciliationEvidence` to `ReportingDeploymentCapabilityDto` and the browser `ReportingDeploymentCapability` contract. The additive, default-backed field makes the mandatory PostgreSQL reconciliation-evidence authority independently visible instead of requiring clients to parse a component id; older payloads deserialize it as `false`, preserving fail-closed behavior.
- 2026-07-26: Corrected `GET /api/workstation/reporting` to be an independent, fail-closed Reporting capability. A successful response is now the direct `WorkstationReportingPayload`, including `deploymentCapability`, rather than a `WorkstationAccountingPayload` wrapper or fabricated fallback; unavailable deployment authority or read state returns `503 Service Unavailable`. `GET /api/workstation/accounting` remains a `WorkstationAccountingPayload` and may continue embedding its Reporting summary. This is an intentional response-shape correction at the Reporting route: the in-repo browser client accepts both the direct payload and the legacy accounting envelope during the normal two-minor-release or 60-day migration window, while external consumers must adopt the direct shape and handle `503` as a deployment blocker.
- 2026-07-26: Added `canonicalDeliveries` to `WorkstationReportingPayload`, with durable job identity, immutable release binding, and append-only provider/portal receipt projections from the tenant-scoped PostgreSQL reporting authorities. `deliveryAttempts` remains an optional compatibility-only field for legacy file-backed report packs and is empty in production composition; older clients may ignore `canonicalDeliveries`, while authority-aware clients should use it. `GET /api/fund-structure/reporting/runs` now returns the same canonical run/delivery history under a bound tenant/company scope. Legacy report-pack history/detail reads remain available only when their legacy repository is explicitly registered and otherwise return `410 Gone` with canonical-route guidance.
...
- 2026-08-03: Added server-resolved `CoveredCallRunScope` overloads to `ICoveredCallBacktestService` and tenant/company ownership to covered-call persistence and routes. The prior unscoped interface overloads remain source-visible and obsolete but deliberately throw `NotSupportedException`; silently routing them to a global run store would recreate a cross-tenant disclosure. Browser consumers keep the HTTP route shapes and use authenticated workstation context. Run/list/status/result reads accept `ViewStrategies` or `ManageStrategies`; start/cancel require `ManageStrategies`. Start now applies item/value/aggregate evidence budgets and requires a strict production-shaped `evidence://evidence-vault/{vaultId}` whose manifest resolves in the authenticated scope before queue admission. Retained runs from the former global `covered-call-overwrite` namespace are not automatically exposed or rebound because their ownership cannot be proven; operators must rerun the backtest under the intended authenticated company. There is no WPF covered-call consumer. These are security fail-closed behavior breaks and require release-manager acknowledgement under the emergency-break policy before release.
- 2026-08-03: Backtest Studio and Strategy Designer declarations now fail closed unless they include a nonblank strategy identity, at least one operator acceptance requirement, and at least one categorized retained URI reference. `BacktestStudioRunOrchestrator.StartAsync` throws `ArgumentException` before invoking an engine when a direct caller omits that declaration set. Existing Strategy Designer JSON remains parseable because the new lists are default-backed, but the workstation route now returns `400` when they are absent; it also requires exactly one captured canonical result, while the current production compiler captures none, so that entrypoint is intentionally unavailable rather than reporting false success. The established positional constructors and `Deconstruct` methods remain source-compatible. `StrategyRunEntry.Start` retains its established overload slots; evidence-aware creation uses `StartWithEvidence`, and durable replay accepts verified legacy, v2, or evidence-bound v3 input hashes without allowing immutable inputs to change during canonicalization. These are intentional fail-closed runtime behavior breaks and require release-manager acknowledgement under the emergency-break policy before release.
- 2026-08-03: `StrategyRunDetail` adds a default-empty read-only `AcceptanceChecklist`, and `StrategyRunPromotionSummary` adds nullable `PromotedAt`; older JSON consumers may ignore both. A checklist item becomes Ready only from a durable approved promotion with operator/time/audit authority, its canonical checklist id, keyed evidence exactly matching the retained source run, and an exact same-scope Paper child with matching parent and strategy identity. Existing promotion route shapes are unchanged, but Backtest-to-Paper approvals that omit the four canonical checklist ids or keyed source evidence now return `400`, and the Strategy browser action uses `/api/promotion/approve` rather than generic paper-session creation. Strategy review packets now require tenant+company scope plus `ViewStrategies` or `ManageStrategies`; Trading resolves scoped run lineage. These are intentional fail-closed behavior breaks and require release-manager acknowledgement.
- 2026-08-03: Added tenant/company scope to Evidence Vault identities, manifests, documents, query DTOs, and the scoped `IEvidenceArtifactStore` read/review overloads. Legacy unscoped interface and concrete `FileEvidenceArtifactStore` method slots remain source-visible, obsolete, and fail closed. All new manifest/intake writes require complete scope at the storage boundary. Existing JSON shapes remain parseable, but Evidence Vault search, list, manifest, packet/graph, and review operations return no record or reject the operation when authoritative scope is absent or mismatched. HTTP reads require `ViewReporting`; validation/export/intake require `ManageReporting`; document review requires `ApproveReporting`, and actor/scope fields are server-owned. Previously retained identities without provable tenant/company ownership are intentionally invisible; do not infer or bulk-backfill ownership. Re-ingest or re-export the original evidence under the authenticated tenant/company, then relink the resulting vault identity. External HTTP intake no longer accepts server-local file paths or caller-supplied initial extraction/reviewer authority; upload bytes (or use the trusted internal statement-import adapter) and perform review through the authenticated review route. Browser consumers were updated; WPF has no direct Evidence Vault intake consumer. This is a security fail-closed behavior break and requires release-manager acknowledgement under the emergency-break policy before release.
- 2026-08-03: Statement-to-Evidence-Vault projection now treats retained artifact bytes as the replay authority and can reconstruct the canonical statement projection after the original local source disappears. Route and DTO shapes remain unchanged. A legacy statement record without readable retained bytes cannot be promoted by path fallback and must be re-imported so a new scoped vault identity and durable byte authority are retained.
- 2026-08-29: Corporate-action source decisions are now gated on the authoritative scope fan-out authority instead of a hard-disabled constant. Route and DTO shapes are unchanged. `POST /api/security-master/corporate-actions/source-proposals/{proposalId}/reject` now requires workstation tenant/company scope like its accept counterpart, and both decision routes answer `503` with the standing fan-out reason where no authority is composed — the same status and detail the routes returned before, so existing clients see no change in that posture. Where the authority is composed, a decision is applied only when the affected set is exactly one scope owned by the caller; a fan-out that reaches another tenant or none of the caller's scopes answers `403` (`corporate_action_scope_mismatch`) and one spanning several scopes answers `422` (`corporate_action_downstream_authority_required`). Narrow acceptance scope fields (fund, account, portfolio, custody, ledger book, period, basis, currency, jurisdiction) remain caller-forbidden and are now server-resolved from the assignment authority and stamped on the acceptance command, so retained cases carry exact scope that callers never asserted. `IFundAccountStore` adds `QueryAccountsAcrossTenantsAsync`, a default-throwing member that only the fan-out authority calls; existing implementations remain source-compatible and fail closed rather than returning a caller-scoped list that would read as a complete cross-tenant answer.
