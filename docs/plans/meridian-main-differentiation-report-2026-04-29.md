...
From the user-supplied design-system archive, I am also assuming that the active commercial UI direction is browser-first for new operator workflows, with WPF retained as a Windows shell and compatibility surface. I also infer that the intended web workspaces are Overview, Research, Trading, Data Operations, and Governance. Because those files are local artifacts rather than publicly searchable sources, I treat them as context rather than independently verifiable public evidence.
