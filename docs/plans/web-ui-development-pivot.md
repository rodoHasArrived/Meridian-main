...
strings centralized in the dashboard endpoint catalog so Settings, readiness, portfolio, run-history,
workflow-summary, and fetch helpers share the same current workspace contract. Evidence Workbench
source-workflow links should prefer packet subject routes, but fall back to shared page-tag routing
when the evidence payload omits a direct route. Settings capability coverage should keep concrete
...
Evidence Workbench packet validation/export commands and Trading readiness refreshes should keep
their revision guards so late async completions cannot overwrite a newer route selection or
readiness payload.
