...
names. Cross-screen workflow links should normalize local `/workstation/...` routes, legacy
...
symbol/quote refreshes, Accounting Security Master drill-ins, and Settings Alpaca credential
actions should keep their active-workstream and mounted/revision guards so late async completions
cannot overwrite newer operator state or repopulate hidden panels.
