...
5. Move Wave 2 cockpit acceptance into the web UI: session restore, replay verification, execution controls, promotion rationale, and operator work items should all consume shared workstation endpoints. Current Trading cockpit evidence includes a refreshable readiness-contract summary for overall, paper-operation, brokerage-sync, and explicit UTC as-of posture plus a link into the read-only console.
   ...
   - Trading Recent Fills now uses the shared dense-table selected-row contract with a fill detail panel, so operators can inspect execution venue, price, linked order, provider context, and timestamp with keyboard-accessible row activation instead of scanning a static table.
6. Move data and governance operator workflows next: Data owns provider posture, backfill preview/trigger, export readiness, watchlists, live quotes, reference-data projection review, and data-quality handoffs, while Accounting owns Security Master coverage, search-result drill-in, details/lots/operator override review, and print/export packet review alongside reconciliation dense-table review, ledger review, close workflow previews, evidence packet actions, and report-line provenance previews. Evidence packet actions now have an initial browser path through `/reporting/evidence`, with links from Trading readiness, Strategy/Portfolio run context, Accounting reconciliation review and dense-table detail selection, and Reporting report-pack context; keep expanding that path through shared evidence subjects before adding page-local evidence state. Portfolio run evidence actions and Reporting workbench evidence actions are view-model-owned so selected run/profile context determines the route contract instead of JSX-local string construction. Export preview and analysis-export fallbacks should preserve the actual generated artifact names: symbol CSV files use stable hash suffixes to avoid collisions, and generated Notebook, Parquet, Excel, HDF5, ClickHouse, and Lean sidecars should point at those exact CSV filenames. The Settings workspace now carries a backend capability coverage panel, view-model-owned header posture chips, and an Alpaca setup checklist that guides operators from paper endpoint selection through account verification into Trading readiness while requiring explicit acknowledgement before any live endpoint credential test. Keep those panels current whenever new shared backend functionality is meant to be browser-accessible.
  - The Data watchlist now includes preset quick-add symbol packs backed by the same bulk symbol API as manual entry, so first-run operators can move from an empty table to live quote validation without guessing an initial universe.
   ...
   - Research promotion-to-paper-session controls now expose view-model-owned disabled reasons for unevaluated runs, ineligible runs, missing/invalid initial cash, missing review acknowledgement, and in-flight session creation, so operators can understand why a paper validation step is blocked without trial-and-error. After a run evaluates as eligible, the browser keeps paper-session creation disabled until the operator acknowledges the promotion gates and paper-capital impact.
  ...
  - Covered Call chain preview now uses the shared dense-table plus selected-detail contract with fixture-backed run history and safe chain-preview POST support, so `/strategy/covered-call` remains demo-ready without a live API host while real run creation stays unmocked. Covered Call run history also uses view-model-owned UTC labels, dense-table row activation, selected/opening-run state with stale-result guards, retry/loading feedback when saved runs fail or reload, guarded endpoint builders for run detail/status/cancel routes, and the shared dashboard API helpers for abort propagation plus backend problem-detail diagnostics.
  - Quant Notebook helper APIs now route notebook cell execution through the Quant run path and historical-bar fetches through shared data APIs, so notebook support remains an extension of the existing Quant Lab/data contracts rather than a parallel execution lane.
   ...
   - Accounting Security Master identifier-conflict loads now expose a view-model-owned refresh/retry command, so trust-critical reference-data failures remain recoverable without leaving the Security Master workflow.
   ...
   - Accounting Security Master schedules are the Wave 1 reference-workbench rollout target: add dashboard-local cash-flow/factor rows, expected-versus-actual variance review, selected-event detail, controls, validation, notes, and audit evidence on `/accounting/security-master` before adding any shared schedule API or restyling other routes. Wave 2 can apply the same dense master-detail pattern to Accounting reconciliation, Data reference coverage, and Portfolio ledger/position evidence screens when those workflows benefit from it; Wave 3 can reach Reporting, Strategy, and Trading only for real schedule/evidence workflows.
   - Evidence Workbench manifest exports now return a retained `/workstation/evidence/...` route that the host serves from `DataRoot/workstation/evidence`, and the browser success state exposes a view-model-owned open-manifest action for immediate audit review.
...
reconciliation, reporting export, provider, backfill, symbol, quality, market-data, Quant Lab,
Covered Call, and fetch helpers share the same current workspace contract. Dynamic route builders should reject blank
...
Calibration summary evidence should also stay recoverable on the reconciliation route: tolerance
profile rows use the shared dense-table/detail-panel contract, selection stays in the view model,
and failed calibration loads expose a view-model-owned retry command with stale-response guards so
operators are not stranded on a dead-end alert.
