...
| --- | --- | --- | --- |
| Replay confidence | **Completed (implementation); awaiting fresh dated packet per release** | Shared readiness models stale replay evidence and work-item IDs (`paper-replay-stale-{sessionId}`), replay audits persist compared counts and mismatch reason, and the Wave 2 evidence runbook now defines `verify -> stale -> re-verify` capture steps. | Generate and archive the current-release date-stamped evidence packet from the runbook sequence. |
