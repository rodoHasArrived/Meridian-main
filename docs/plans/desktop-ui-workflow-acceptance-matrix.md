...
- 2026-05-27: Lane B browser-parity support evidence now keeps the shared `FundTrialBalance` workflow target aligned with the WPF trial-balance route by resolving it to browser `/accounting/ledger` instead of the accounting root. Focused validation: `npm --prefix src/Meridian.Ui/dashboard run test:vitest -- src/lib/workspace.test.ts src/components/meridian/workspace-nav.view-model.test.ts src/screens/governance-screen.view-model.test.ts src/screens/governance-screen.test.tsx` and `dotnet test tests/Meridian.Tests/Meridian.Tests.csproj --filter "FullyQualifiedName~WorkflowLibraryEndpointTests" --logger "console;verbosity=normal"`. This is a drift-risk closure for workflow routing; it is not a full W3 continuity acceptance claim.
