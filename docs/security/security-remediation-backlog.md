...
- **Required code/tests (remaining slices to close the gap / enable multi-tenant deployment):**
  ...
  - **Slice 3 (done — ledger read boundary):** ✔ gate the fund-scoped ledger read routes by fund ownership via the `RequireFundProfileTenantScope()` endpoint filter, refusing a positively-foreign `fundProfileId` with `403` (fail-open otherwise). Regression tests prove a foreign fund is rejected, an own/blank fund passes, and a missing guard fails open. Report-pack workflow-record reads and the `ReportPackSecurityLineIndex` cross-fund `Lookup` are not endpoint-reachable today (internal callers use the fund-scoped `LookupByFund`), so they carry no current cross-tenant read path. _Slice 3b (done):_ ✔ applied the same filter to the remaining `fundProfileId`-query workstation reads — accounting-system (cert profile, migration run artifacts, migration worker plans, import-latest, reconciliation-latest, mapping-profiles, export-packages), operations-continuity command center + private-capital close cockpit, and workflow-summary. Reporting structured-export is `exportId`-keyed and already company/report-group access-controlled via `ReportAccessQueryContext` (no `fundProfileId`-query gap). A wiring regression test proves a foreign `fundProfileId` is refused with `403` on a representative accounting-system route before the handler runs, and that a caller without the route's read permission is not given an ownership oracle.
  ...
  - **Slice 4c (remaining — storage columns + fail-closed)** — add `tenant_id` partition columns + read predicates to the fund-scoped stores (closes the slice-3 alternate-identifier / unscoped-list residual); **require a non-null server-resolved tenant on the fund-scoped write paths (fail closed)** so a tenantless-but-authenticated session — the legacy `MDC_USERNAME` admin profile has no `CompanyId`, and `Resolve` then returns a null tenant which downstream filters treat as global/all scope — cannot create or evaluate accounting artifacts in the global namespace (a contract change deliberately deferred from 4a, which only stopped the body override; it would 403 today's supported tenantless accounting sessions, so it belongs with the storage-enforced reclassification); reclassify this item from deployment-boundary-enforced to storage-enforced.
