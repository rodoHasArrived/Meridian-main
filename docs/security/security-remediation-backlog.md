...
- **In place now (slice 2 — bind-on-write + read gate):** the Security Master workbench field-edit route binds the body-supplied fund to the caller's server-resolved tenant via `IFundProfileTenancyRegistry.BindAsync` only **after** the governed write succeeds (trust-on-first-use), so a rejected/failed write can never squat a fund id (`ClaimFundOwnershipAsync` in `WorkstationEndpoints.SecurityMasterWorkbench.cs`). The post-commit claim runs on a request-independent cancellation token so a client disconnect between commit and bind cannot leave the committed fund unbound. On the read side, `SecurityMasterWorkbenchQueryService.LoadFundRunsAsync` now consults the registry through `IsFundAccessibleToCurrentTenantAsync` (using the request's resolved tenant via `IHttpContextAccessor`): a fund the registry positively reports as owned by a **different** tenant yields no runs and no exposures, and its downstream impact is reported as **withheld/`Unknown`** (not `None`): an empty `None` impact would let low-risk mutation gates such as bulk conflict resolution (`BulkResolveConflictsAsync`, eligible only on `None`/`Low`) treat a foreign scope as *safe* instead of forbidden, so a foreign fund instead reads as `Unknown`, which is non-eligible while still disclosing zero runs/exposures (`BuildWithheldFundImpact`). Unbound/legacy funds, a blank fund, a request without tenant scope, or an unavailable registry all **fail open** to the single-company-per-deployment boundary, so the restatement path never drops impact on mere uncertainty and single-tenant deployments are unaffected.
...
- **Required code/tests (remaining slices to close the gap / enable multi-tenant deployment):**
  ...
  - **Slice 4** — stop accepting client-body tenant overrides (`?? request.TenantId` / `?? request.CompanyId`) on accounting routes once an ambient server tenant context exists; make the trust-on-first-use ownership claim **atomic with the governed write commit** (reserve/verify ownership as a write precondition so a concurrent first-edit by another tenant cannot commit against a fund it does not win — see slice 2's "concurrent first-use race" residual); reclassify this item from deployment-boundary-enforced to storage-enforced.
