...
- **Required code/tests (remaining slices to close the gap / enable multi-tenant deployment):**
  ...
  - **Slice 4 (remaining: 4b atomic claim, 4c storage columns)** — make the trust-on-first-use ownership claim **atomic with the governed write commit** (reserve/verify ownership as a write precondition so a concurrent first-edit by another tenant cannot commit against a fund it does not win — see slice 2's "concurrent first-use race" residual); add `tenant_id` partition columns + read predicates to the fund-scoped stores (closes the slice-3 alternate-identifier / unscoped-list residual); **require a non-null server-resolved tenant on the fund-scoped write paths (fail closed)** so a tenantless-but-authenticated session — the legacy `MDC_USERNAME` admin profile has no `CompanyId`, and `Resolve` then returns a null tenant which downstream filters treat as global/all scope — cannot create or evaluate accounting artifacts in the global namespace (a contract change deliberately deferred from 4a, which only stopped the body override; it would 403 today's supported tenantless accounting sessions, so it belongs with the storage-enforced reclassification); reclassify this item from deployment-boundary-enforced to storage-enforced.
