...
- **In place now (slice 2 — bind-on-write + read gate):** the Security Master workbench field-edit route binds the body-supplied fund to the caller's server-resolved tenant via `IFundProfileTenancyRegistry.BindAsync` only **after** the governed write succeeds (trust-on-first-use), so a rejected/failed write can never squat a fund id (`ClaimFundOwnershipAsync` in `WorkstationEndpoints.SecurityMasterWorkbench.cs`). On the read side, `SecurityMasterWorkbenchQueryService.LoadFundRunsAsync` now consults the registry through `IsFundAccessibleToCurrentTenantAsync` (using the request's resolved tenant via `IHttpContextAccessor`): a fund the registry positively reports as owned by a **different** tenant yields no runs — and therefore no downstream portfolio/ledger/reconciliation/report-pack impact. Unbound/legacy funds, a blank fund, a request without tenant scope, or an unavailable registry all **fail open** to the single-company-per-deployment boundary, so the restatement path never drops impact on mere uncertainty and single-tenant deployments are unaffected.
