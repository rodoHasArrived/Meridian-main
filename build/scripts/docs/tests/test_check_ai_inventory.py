...
class CheckAiInventoryTests(unittest.TestCase):
    ...
    def test_check_catalog_drift_allows_deferred_wpf_operator_surface_language(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            write_required_docs(root, "Shared AI documentation docs/ai/")
            write_ui_platform_policy_docs(root)
            write(
                root / ".github" / "prompts" / "sample.prompt.yml",
                "Meridian includes active browser workstation and retained WPF support.\n",
            )
