...
EXPORTER_SOURCE = Path("src/Meridian.Application/Http/Endpoints/StatusEndpointHandlers.cs")
REGISTRY_EXPORT_CALL = "CollectAndExportAsTextAsync"


def check_registry_is_served(root: Path) -> list[str]:
    """Return findings when /metrics does not serve the prometheus-net default registry.

    Every other check in this gate reads `Metrics.Create*` declarations and calls the result
    "emitted". That word is only true if the exposition endpoint actually serves the registry
    those declarations land in. It did not: `GetPrometheusMetrics` hand-wrote ten series into a
    StringBuilder and stopped, so 81 declarations were incremented at runtime and never scraped,
    and this gate would have reported a clean contract over alerts that could not fire.

    Without this check the gate's central claim is unfounded, so it is checked rather than
    assumed.
    """
    path = root / EXPORTER_SOURCE
    if not path.is_file():
        return [
            f"{EXPORTER_SOURCE} not found, so this gate cannot confirm that /metrics serves the "
            "prometheus-net registry. Every 'emitted metric' below would be an assumption."
        ]

    try:
        text = strip_comments(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError) as exc:
        return [f"{EXPORTER_SOURCE} could not be read ({exc}), so registry exposure is unverified."]

    if REGISTRY_EXPORT_CALL not in text:
        return [
            f"{EXPORTER_SOURCE} does not call {REGISTRY_EXPORT_CALL}, so /metrics serves only the "
            "hand-written legacy series and every metric declared through Metrics.Create* is "
            "never scraped. Alerts naming them cannot fire regardless of system state."
        ]
...
def run(root: Path) -> tuple[list[Finding], dict[str, int]]:
    ...
    # Checked first: every finding below reasons about "emitted" metrics, and that word is
    # only meaningful if the endpoint serves the registry those declarations register into.
    exporter_findings = [
        Finding("exporter", str(EXPORTER_SOURCE), message)
        for message in check_registry_is_served(root)
    ]
    ...
    counts = {
        "exporter_findings": len(exporter_findings),
