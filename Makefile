...
help: ## Show this help message
	...
	@grep -hE '^[a-zA-Z_-]+:.*?## .*$$' $(MAKEFILE_LIST) | grep -E '^pre-pr|^pre-pr-full' | awk 'BEGIN {FS = ":.*?## "}; {printf "  $(GREEN)%-28s$(NC) %s\n", $$1, $$2}'
	@echo ""
	@echo "$(BLUE)AI Required Quality Gates:$(NC)"
	...
	@grep -hE '^[a-zA-Z_-]+:.*?## .*$$' $(MAKEFILE_LIST) | grep -E '^ai-audit|^ai-report|^ai-codex-skills-check|^ai-mdif-context|^ai-docs-freshness|^ai-docs-drift|^ai-docs-sync-report|^ai-docs-map|^ai-plan-checklists-check|^ai-arch-check-summary|^ai-arch-check-json' | awk 'BEGIN {FS = ":.*?## "}; {printf "  $(GREEN)%-28s$(NC) %s\n", $$1, $$2}'
	@echo ""
