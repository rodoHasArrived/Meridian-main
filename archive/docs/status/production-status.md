...
**Status:** Development / Pilot Ready - Wave 1 trust gate, Wave 2 paper-trading cockpit, Wave 3 shared-run continuity, and Wave 4 governance/fund-operations close/report baselines are closed as of 2026-05-29. Browser and WPF are both active operator UI lanes and must continue to consume shared contracts, endpoints, and read models. The W4 closure packet combines operations-continuity endpoint/workflow coverage, reconciliation casework, governed report-pack workflow/provenance, browser Accounting/Reporting parity, WPF `Category=W4Acceptance` coverage, `FundOpsCloseLaneScenarioTests`, and `PilotAcceptanceHarnessTests` with `w4Acceptance.isDone=true`. These preserve the closed W1-W4 baselines; they are not Evidence Vault/report-line-provenance product completion beyond the current manifest/export support, Backtest Studio, first-class UFL package graduation, or live-readiness exits.
...
`/api/workstation/operations/continuity*` routes now provide a concrete account-period close-lane
API. This is support evidence for Wave 4 governance and fund-operations productization; browser
workflow acceptance, external statement/custodian intake, generalized durable casework, report
publication controls, and operator-approved calibration remain open.
...
reopen the closed W2/W3 baselines; W4 closes later in the 2026-05-29 acceptance packet.
...
2026-05-29 W4 close note: the active repository now has green shared/browser/source-route evidence
for W4 operations-continuity, reconciliation casework, report-pack workflow/provenance, Security
Master endpoint mapping, fund-structure report-pack/cash-flow endpoints,
`FundOpsCloseLaneScenarioTests`, and `PilotAcceptanceHarnessTests`. Browser W4 tests cover
operations-continuity, governance, and reporting surfaces, route-parity checks keep shared workflow
targets aligned across browser and WPF, and the WPF Lane C `Category=W4Acceptance` filter anchors
retained-desktop coverage. This is the W4 closure packet; future close/report changes are baseline
maintenance unless they explicitly belong to W5 or W6.
...
Meridian already has working ingestion, storage, replay, backtesting, provider orchestration,
export tooling, shared workstation endpoints, an active web workstation dashboard, retained
WPF/local API support surfaces, and a delivered Security Master baseline. The main product gap is
no longer missing foundations. It is the remaining work required to turn those foundations into a
coherent evidence-backed investment operations product with trustworthy provider evidence, a
dependable paper-trading lane, one shared run-centered model, accounting and reconciliation
continuity, and governed outputs.

The current repository reinforces that direction rather than changing it. The browser workstation now includes app-shell view-state handling for loading, partial-degradation, retry, and bootstrap-failure panels, query-string route-focus keys for subject/symbol handoffs, plus a Research run-library first cut with retained-run review, two-run compare/diff readiness, selected run-detail inspection, promotion-history decision detail, operator-visible command errors, focused tests, and refreshed built workstation assets. It also now loads shared workflow library and preset commands into grouped command-palette sections with hash-aware matching for the Alpaca provider-setup anchor, replaces placeholder routing with dedicated Portfolio/Reporting/Settings/Data task panels, projects brokerage connection and household portfolio evidence into Portfolio with account-filter focus restoration plus provider-repair, Trading-readiness, and Trading-cockpit next actions, shows report-pack task/approval-target state in Reporting with selected-profile preview/export-analysis actions and concrete GET endpoint links separated from reference-only POST export mutations, exposes `/reporting/evidence` for packet completeness, graph lineage, validation, and manifest export, exposes backend capability coverage and Alpaca paper-key verification in Settings, adds provider-setup success actions that route to live quotes, backfill preview, Trading readiness, or Security Master coverage based on selected capabilities, derives Security Master print-packet readiness in Data, adds Data live quote/order-book/watchlist workflows with multi-symbol quote snapshots, selectable recent-trade detail inspection, staged click-to-trade order tickets that hand accepted or rejected submissions to Trading readiness, view-model-owned Watchlist empty/no-quote/busy labels, intraday and historical price charts, exposes a gated Quant Lab browser surface backed by `/api/quant/run`, `/api/quant/parameters`, and `/api/quant/templates` with plot geometry/accessibility state split into a reusable view model, and projects Accounting reconciliation run selection, open-break copy, no-host break-queue fixture coverage, and empty-detail guidance through a view-model-owned dense detail queue. It trims duplicated rail/header current-workspace context, adds Overview portfolio-at-a-glance plus view-model-owned status-banner and refresh-command presentation, covers menu-linked web screenshot routes, and seeds no-host quote/trade/depth/symbol/snapshot/historical fixtures so `/data/quotes?symbol=AAPL` opens as a populated demo workbench. Security Master support now also includes browser search-result selection, identity drill-ins, details/lots tracking with selectable lot-detail state, server-side operator overrides, conflict refresh/retry command state, and UFL/reference-data projections through shared DTOs, stores, migrations, and `/api/reference-data/*` endpoints for the expanded asset-class set. Command-palette and mega-menu focus handling is also stronger, Trading readiness as-of values now render as explicit UTC labels and the Trading loading panel now has view-model-owned busy/live-region state, and shared UI-service persistence now uses `AtomicFileWriter` for config, export-preset, and quality-archive JSON writes. The WPF workspace-shell baseline is present through `ShellNavigationCatalog`, workspace shell pages, `MainPageViewModel` orchestration, deep-page hosting, shell-context strips, shell smoke tests, the Trading desk briefing hero with stale-replay count detail, Trading Hours session briefing, OrderBook order-flow posture, the Research desk briefing hero, the Data shell feature module with snapshot/presentation/view-model ownership, Provider Health posture briefing, System Health triage briefing, Notification Center history recovery, Activity Log triage strip with export/clear header actions, the main-shell operator queue button backed by `/api/workstation/operator/inbox` with active-account `fundAccountId` propagation, actionable run review-packet work items, Account Portfolio routing for brokerage-sync blockers, and shell-context attention cues, Messaging Hub delivery-posture state with refresh recency, StrategyRuns filter recovery, route-aware run review-packet work items, BatchBacktest results empty guidance, QuantScript local run-history handoffs, Fund Accounts account-queue/provider-routing/shared-data and balance-evidence briefing state, Storage archive growth/capacity/last-scan posture, focused tests for Batch Backtest, Trading Hours, Position Blotter, Fund Accounts, QuantScript, Notification Center, Welcome, Storage posture, System Health triage behavior, Activity Log triage/export/clear behavior, workspace queue tone styles, workspace shell context-strip behavior, Trading hero state selection, Research run/promotion handoffs, Provider Health posture states, hidden-but-present workflow page-state markers, deterministic isolated workflow restore/build behavior, and local single-instance mutex plus launch-argument forwarding coverage. Workflow-level acceptance still belongs to Waves 2-4 rather than a separate desktop migration milestone.
...
The May 15 browser evidence extends the support surface, but does not change the production
posture. The web workstation now has a Today panel for day P&L, movers, open orders, and fills;
local Price Alerts at `/data/alerts`; a visual Strategy Designer at `/strategy/designer`;
Covered Call chain preview, saved-run history, and trade-timeline detail state at
`/strategy/covered-call`; Quant Notebook cell/data-fetch helpers; selectable Recent Fills detail
state; full-console readiness checkpoint
gating; and Meridian Design System reference workbench/tokenized-color support. These reduce
operator dead ends and improve demo/readiness quality, but cockpit acceptance, durable
reconciliation casework, report-pack lifecycle acceptance, Evidence Vault, report-line provenance,
and controlled live-readiness remain open.
...
| Brokerage gateway framework | Partial | Alpaca, IB, Robinhood, and StockSharp paths exist; Alpaca now has browser Settings paper-key verification, and Robinhood now has separate read-only aggregation support for normalized account/portfolio/activity sync. Broader runtime proof and controlled live-readiness remain open |
...
| Security Master platform seam | Complete | WPF, Research, Trading, Portfolio, Ledger, Reconciliation, and Governance share one authoritative coverage/provenance contract; current support evidence adds browser search-result selection, identity drill-ins, details/lots tracking, server-side operator overrides, and UFL/reference-data projections for the expanded asset-class set |
| Governance product surfaces | Partial | Security coverage, Fund Accounts account/routing/balance-evidence posture, browser Accounting reconciliation dense-table selection with keyboard-expanded detail, no-host break-queue fixture coverage, seeded exception-route/tolerance/sign-off metadata, calibration-summary rollups, operations-continuity close-lane APIs, direct lending, and reporting-adjacent seams are live; broader multi-ledger, cash-flow, browser close acceptance, external statement/custodian intake, durable casework, operator-approved calibration, and governed reporting workflows remain incomplete |
| Web workstation dashboard + WPF/local API surfaces | Partial | The active browser workstation now exposes overview/strategy/trading/portfolio/data/accounting/reporting/settings routes, Research run-library support, gated Quant Lab execution/parameter/template support with testable plot view-model state, Quant Notebook helpers, workflow command-palette commands with hash-aware provider-setup routing, Portfolio brokerage-sync evidence with provider-repair/readiness/cockpit next actions plus selectable position/run-evidence dense tables, Overview portfolio-at-a-glance, richer sidebar/menu/command grouping, menu-linked screenshot routes, Reporting report-pack task state and Evidence Workbench, Settings backend capability/Alpaca connection posture plus provider-setup validation handoffs, Data Security Master print-packet readiness, Data backfill dense-table queue selection with keyboard-expanded detail, browser Accounting reconciliation dense-table rows with keyboard-expanded detail and no-host break-queue fixture coverage, browser Security Master search-result selection, identity drill-ins, details/lots/operator overrides, live quote/order-book/watchlist workflows with view-model-owned no-quote/empty/busy copy, multi-symbol quote snapshots, selectable recent-trade detail inspectors, click-to-trade order-ticket staging, selectable Recent Fills detail state, historical charts, Covered Call chain-preview, saved-run-history, and trade-timeline support, seeded no-host AAPL market-data fixtures, simplified rail/header context, shared reference-data and evidence packet/graph/validation/manifest-export APIs, and hardened command-palette/mega-menu/Portfolio filter focus behavior; WPF surfaces project Trading, Trading Hours, OrderBook posture, Research, Data shell feature-module state, Storage archive posture, Provider Health, System Health, Notification Center, Activity Log, Watchlist pinned-first staging, StrategyRuns, QuantScript, Fund Accounts account and balance-evidence posture, operator queue, and Messaging Hub support surfaces from shared readiness/run/provider/depth/account/calendar/storage/local history/messaging inputs. Shared UI persistence now uses atomic writes for config, export presets, and quality archive snapshots. Workflow hardening, Evidence Vault/report-line provenance, and deeper workflow-first consolidation remain |
...
- active web workstation dashboard plus WPF/local API surfaces that consume shared workstation contracts
...
- governance/fund-operations close and report acceptance is now a closed baseline: the Fund Accounts workbench projects account-queue, provider-routing, shared-data, retained balance-evidence, and ready-for-reconciliation states from loaded evidence; browser Portfolio surfaces brokerage household/account posture; browser Reporting exposes report-pack task state; Security Master has browser search-result selection, identity drill-ins, details/lots/operator overrides plus UFL/reference-data projection support; reconciliation break-queue casework is file-backed with review, resolve/dismiss, audit-history routes, seeded exception-route/tolerance/sign-off metadata, calibration-summary rollups, browser Accounting dense-table selection/empty-state guidance, and no-host break-queue fixture coverage; and the operations-continuity close lane covers broker import/normalization through checklist, reconciliation, approval, close, and reopen via shared workstation routes
- the next governance slices should preserve this W4 baseline by extending shared DTOs, read models, and export seams instead of creating a second governance stack
...
Wave 4 readiness language is now `Done`; continue running the objective gate as a regression suite before changing close/report workflows.
