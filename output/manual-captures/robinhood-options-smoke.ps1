...
function ConvertTo-HashtableDeep {
    ...
    if ($Value -is [System.Collections.IDictionary]) {
        ...
        foreach ($key in $Value.Keys) {
            $hash[$key] = ConvertTo-HashtableDeep -Value $Value[$key]
        }
    ...
    if ($properties.Length -gt 0) {
        ...
        foreach ($property in $properties) {
            $hash[$property.Name] = ConvertTo-HashtableDeep -Value $property.Value
        }
...
function Read-JsonHashtable {
    param([string]$Path)
    return ConvertTo-HashtableDeep -Value (Get-Content -Path $Path -Raw | ConvertFrom-Json)
}
...
function Save-JsonHashtable {
    ...
    $jsonNode = ConvertTo-JsonNodeDeep -Value $Value
    $jsonOptions = [System.Text.Json.JsonSerializerOptions]::new()
    $jsonOptions.WriteIndented = $true
    $json = if ($null -eq $jsonNode) {
        "null"
    } else {
        $jsonNode.ToJsonString($jsonOptions)
    }
    Set-Content -Path $Path -Value $json -Encoding UTF8
}

function ConvertTo-JsonNodeDeep {
    param([object]$Value)
    ...
    if ($Value -is [System.Collections.IDictionary]) {
        $objectNode = [System.Text.Json.Nodes.JsonObject]::new()
        foreach ($key in $Value.Keys) {
            $childNode = Normalize-JsonNode -Value (ConvertTo-JsonNodeDeep -Value $Value[$key])
            if ($childNode -is [System.Text.Json.Nodes.JsonNode] -and $null -ne $childNode.Parent) {
                $childNode = $childNode.DeepClone()
            }

            $propertyName = [string]$key
            $null = $objectNode.Remove($propertyName)
            $objectNode.Add($propertyName, $childNode)
        }
    ...
    if ($Value -is [System.Collections.IEnumerable]) {
        $arrayNode = [System.Text.Json.Nodes.JsonArray]::new()
        foreach ($item in $Value) {
            $childNode = Normalize-JsonNode -Value (ConvertTo-JsonNodeDeep -Value $item)
            if ($childNode -is [System.Text.Json.Nodes.JsonNode] -and $null -ne $childNode.Parent) {
                $childNode = $childNode.DeepClone()
            }
    ...
    if ($properties.Length -gt 0) {
        $objectNode = [System.Text.Json.Nodes.JsonObject]::new()
        foreach ($property in $properties) {
            $childNode = Normalize-JsonNode -Value (ConvertTo-JsonNodeDeep -Value $property.Value)
            if ($childNode -is [System.Text.Json.Nodes.JsonNode] -and $null -ne $childNode.Parent) {
                $childNode = $childNode.DeepClone()
            }

            $null = $objectNode.Remove($property.Name)
            $objectNode.Add($property.Name, $childNode)
        }

        return $objectNode
    }

    return [System.Text.Json.Nodes.JsonValue]::Create([string]$Value)
}
...
function Normalize-JsonNode {
    ...
    if ($Value -is [System.Management.Automation.PSObject] -and
        $Value.BaseObject -is [System.Text.Json.Nodes.JsonNode]) {
...
function New-SmokeWorkspaceData {
    ...
    if (-not $data.ContainsKey("sessionsByFundProfileId") -or
        $null -eq $data["sessionsByFundProfileId"] -or
        -not ($data["sessionsByFundProfileId"] -is [System.Collections.IDictionary])) {
        $data["sessionsByFundProfileId"] = @{}
    }

    $data["sessionsByFundProfileId"][$OperatingContextKey] = $session
    $data["sessionsByFundProfileId"][$FundProfileId] = $session

    $normalizedWorkspaces = @()
    foreach ($workspace in @($data["workspaces"])) {
        $workspaceHash = ConvertTo-HashtableDeep -Value $workspace
        $normalizedWorkspaces += ,$workspaceHash

        if ($workspaceHash["id"] -eq $WorkspaceId) {
            $workspaceHash["lastActivePageTag"] = $PageTag
            $workspaceHash["recentPageTags"] = @($PageTag)
            $workspaceHash["sessionSnapshot"] = $session
        }
    }

    $data["workspaces"] = $normalizedWorkspaces
...
function Invoke-SmokeCase {
    ...
    Save-JsonHashtable -Path (Join-Path $OutputDirectory ("seeded-" + $Case.Name + ".json")) -Value $seededData
    ...
    try {
        ...
        $selectionMarkers = @(
            ...
            "Fund Profile Selection",
            "Choose Fund Profile"
        )
        $pageReady = $null
        try {
            $pageReady = Wait-Until -TimeoutSeconds 8 -FailureMessage "Initial target page marker was not yet visible." -Condition {
                Find-FirstElementByNames -Root $root -Names $Case.ReadyMarkers
            }
        } catch {
            $pageReady = $null
        }
        ...
        if ($null -eq $pageReady) {
            $fundPage = Wait-ForElementByNames -Root $root -TimeoutSeconds 18 -Names $selectionMarkers -FailureMessage "Timed out waiting for fund selection or target page."
            Write-Log "Selection screen detected. Entering the selected operating context."
            try {
                $enterFundButton = Wait-Until -TimeoutSeconds 4 -FailureMessage "Selection page appeared, but the entry button was not found." -Condition {
                    foreach ($candidateName in @("Enter Workstation", "Enter Fund", "Enter")) {
                        $byName = Find-ElementByExactName -Root $root -Name $candidateName
                        if ($null -ne $byName) {
                            return $byName
                        }
                    }

                    return Find-ElementByAutomationId -Root $root -AutomationId "EnterWorkstationButton"
                }
                Invoke-OrClickElement -Element $enterFundButton
            } catch {
                Write-Log "The selection button was not exposed through UI Automation. Sending Enter to the focused window instead."
...
$baseWorkspaceData = if ($null -ne $workspaceDataOriginal) {
    ConvertTo-HashtableDeep -Value ($workspaceDataOriginal | ConvertFrom-Json)
} else {
