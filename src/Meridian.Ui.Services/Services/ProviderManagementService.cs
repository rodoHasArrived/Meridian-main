...
public sealed class ProviderManagementService
{
    ...
    /// <summary>
    /// Gets relationship-aware provider connections.
    /// </summary>
    public async Task<ProviderConnectionsResult> GetProviderConnectionsAsync(CancellationToken ct = default)
    {
        var response = await _apiClient.GetWithResponseAsync<List<ProviderConnectionDto>>(
            UiApiRoutes.ProviderOperationsConnections,
            ct);

        if (response.Success)
        {
            return new ProviderConnectionsResult
            {
                Success = true,
    ...
    /// <summary>
    /// Creates or updates a relationship-aware provider connection.
    /// </summary>
    public async Task<ProviderConnectionMutationResult> UpsertProviderConnectionAsync(
        CreateProviderConnectionRequest request,
        CancellationToken ct = default)
    {
        var response = await _apiClient.PostWithResponseAsync<ProviderConnectionDto>(
            UiApiRoutes.ProviderOperationsConnections,
            request,
            ct);

        return new ProviderConnectionMutationResult
        {
            Success = response.Success && response.Data is not null,
            Connection = response.Data,
            Message = response.Success ? "Provider connection saved." : null,
            Error = response.Success ? null : response.ErrorMessage ?? "Failed to save provider connection."
        };
    }
    ...
    /// </summary>
    public async Task<OperationResult> DeleteProviderConnectionAsync(string connectionId, CancellationToken ct = default)
    {
        var route = UiApiRoutes.WithParam(UiApiRoutes.ProviderOperationsConnectionById, "connectionId", connectionId);
        var response = await _apiClient.DeleteWithResponseAsync<OperationResult>(route, ct);

        return new OperationResult
        {
            Success = response.Success,
            Message = response.Success ? "Provider connection deleted." : null,
            Error = response.Success ? null : response.ErrorMessage ?? "Failed to delete provider connection."
        };
    }

    /// <summary>
    /// Gets relationship-aware provider bindings.
    /// </summary>
    public async Task<ProviderBindingsResult> GetProviderBindingsAsync(CancellationToken ct = default)
    {
        var response = await _apiClient.GetWithResponseAsync<List<ProviderBindingDto>>(
            UiApiRoutes.ProviderOperationsBindings,
            ct);

        if (response.Success)
        {
            return new ProviderBindingsResult
            {
                Success = true,
    ...
    /// <summary>
    /// Creates or updates a provider capability binding.
    /// </summary>
    public async Task<ProviderBindingMutationResult> UpsertProviderBindingAsync(
        UpdateProviderBindingRequest request,
        CancellationToken ct = default)
    {
        var response = await _apiClient.PostWithResponseAsync<ProviderBindingDto>(
            UiApiRoutes.ProviderOperationsBindings,
            request,
            ct);

        return new ProviderBindingMutationResult
        {
            Success = response.Success && response.Data is not null,
            Binding = response.Data,
            Message = response.Success ? "Provider binding saved." : null,
            Error = response.Success ? null : response.ErrorMessage ?? "Failed to save provider binding."
        };
    }

    /// <summary>
    /// Deletes a provider capability binding.
    /// </summary>
    public async Task<OperationResult> DeleteProviderBindingAsync(string bindingId, CancellationToken ct = default)
    {
        var route = UiApiRoutes.WithParam(UiApiRoutes.ProviderOperationsBindingById, "bindingId", bindingId);
        var response = await _apiClient.DeleteWithResponseAsync<OperationResult>(route, ct);

        return new OperationResult
        {
            Success = response.Success,
            Message = response.Success ? "Provider binding deleted." : null,
            Error = response.Success ? null : response.ErrorMessage ?? "Failed to delete provider binding."
        };
    }

    /// <summary>
    /// Gets effective provider safety policies.
    /// </summary>
    public async Task<ProviderPoliciesResult> GetProviderPoliciesAsync(CancellationToken ct = default)
    {
        var response = await _apiClient.GetWithResponseAsync<List<ProviderPolicyDto>>(
            UiApiRoutes.ProviderOperationsPolicies,
            ct);

        if (response.Success)
        {
            return new ProviderPoliciesResult
            {
                Success = true,
                Policies = response.Data?.ToList() ?? []
            };
        }
        ...
        {
            Success = false,
            Error = response.ErrorMessage ?? "Failed to load provider policies."
        };
    }

    /// <summary>
    /// Previews a provider route using the relationship-aware router.
    /// </summary>
    public async Task<ProviderRoutePreviewQueryResult> PreviewRouteAsync(RoutePreviewRequest request, CancellationToken ct = default)
    {
        var response = await _apiClient.PostWithResponseAsync<RoutePreviewResponse>(
            UiApiRoutes.ProviderOperationsRoutePreview,
            request,
            ct);

        return new ProviderRoutePreviewQueryResult
        {
            Success = response.Success && response.Data is not null,
            Preview = response.Data,
            Error = response.Success ? null : response.ErrorMessage ?? "Failed to preview provider route."
        };
    }

    /// <summary>
    /// Gets recent provider route history.
    /// </summary>
    public async Task<ProviderRouteHistoryResult> GetProviderRouteHistoryAsync(CancellationToken ct = default)
    {
        var response = await _apiClient.GetWithResponseAsync<List<RoutePreviewResponse>>(
            UiApiRoutes.ProviderOperationsRouteHistory,
            ct);

        if (response.Success)
        {
            return new ProviderRouteHistoryResult
            {
                Success = true,
                History = response.Data?.ToList() ?? []
            };
        }
        ...
        {
            Success = false,
            Error = response.ErrorMessage ?? "Failed to load provider route history."
        };
    }

    /// <summary>
    /// Gets provider routing presets.
    /// </summary>
    public async Task<ProviderPresetsResult> GetProviderPresetsAsync(CancellationToken ct = default)
    {
        var response = await _apiClient.GetWithResponseAsync<List<ProviderPresetDto>>(
            UiApiRoutes.ProviderOperationsPresets,
            ct);

        if (response.Success)
        {
            return new ProviderPresetsResult
            {
                Success = true,
                Presets = response.Data?.ToList() ?? []
            };
        }
        ...
        {
            Success = false,
            Error = response.ErrorMessage ?? "Failed to load provider presets."
        };
    }

    /// <summary>
    /// Applies a provider routing preset.
    /// </summary>
    public async Task<ProviderPresetApplyResult> ApplyProviderPresetAsync(string presetId, CancellationToken ct = default)
    {
        var response = await _apiClient.PostWithResponseAsync<ProviderPresetDto>(
            UiApiRoutes.ProviderOperationsApplyPreset,
            new ApplyProviderPresetRequest(presetId),
            ct);

        return new ProviderPresetApplyResult
        {
            Success = response.Success && response.Data is not null,
            Preset = response.Data,
            Message = response.Success ? "Provider preset applied." : null,
            Error = response.Success ? null : response.ErrorMessage ?? "Failed to apply provider preset."
        };
    }

    /// <summary>
    /// Gets provider certifications.
    /// </summary>
    public async Task<ProviderCertificationsResult> GetProviderCertificationsAsync(CancellationToken ct = default)
    {
        var response = await _apiClient.GetWithResponseAsync<List<ProviderCertificationDto>>(
            UiApiRoutes.ProviderOperationsCertifications,
            ct);

        if (response.Success)
        {
            return new ProviderCertificationsResult
            {
                Success = true,
                Certifications = response.Data?.ToList() ?? []
            };
        }
        ...
        {
            Success = false,
            Error = response.ErrorMessage ?? "Failed to load provider certifications."
        };
    }

    /// <summary>
    /// Runs provider certification for a saved connection.
    /// </summary>
    public async Task<ProviderCertificationMutationResult> RunProviderCertificationAsync(
        string connectionId,
        CancellationToken ct = default)
    {
        var response = await _apiClient.PostWithResponseAsync<ProviderCertificationDto>(
            UiApiRoutes.ProviderOperationsCertificationRun,
            new RunCertificationRequest(connectionId),
            ct);

        return new ProviderCertificationMutationResult
        {
            Success = response.Success && response.Data is not null,
            Certification = response.Data,
            Message = response.Success ? "Provider certification completed." : null,
            Error = response.Success ? null : response.ErrorMessage ?? "Failed to run provider certification."
        };
    }

    /// <summary>
    /// Gets provider trust snapshots.
    /// </summary>
    public async Task<ProviderTrustSnapshotsResult> GetProviderTrustSnapshotsAsync(CancellationToken ct = default)
    {
        var response = await _apiClient.GetWithResponseAsync<List<ProviderTrustSnapshotDto>>(
            UiApiRoutes.ProviderOperationsTrust,
            ct);

        if (response.Success)
        {
            return new ProviderTrustSnapshotsResult
            {
                Success = true,
