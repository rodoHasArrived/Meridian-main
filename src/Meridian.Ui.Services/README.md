...
Statement-run creation now performs broker/custodian CSV intake through the shared reconciliation API service, persists generated break records, opens durable casework with owner, SLA due date, comments, attachments, and explanation metadata, and returns the linked cases through `StatementRunDto` for browser and WPF operators.
