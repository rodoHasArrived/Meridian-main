    ...
    ICorporateActionAdjustmentService? corporateActionAdjustment = null)
{
        ...
        CancellationToken ct = default)
    {
        ...
        // Aggregate all closed lots from snapshots (TakeSnapshot clears account.ClosedLots
        // after each daily snapshot, so the full audit trail lives in the snapshots).
        var allClosedLots = allSnapshots
            .SelectMany(static s => s.ClosedLots ?? [])
            .ToArray();
