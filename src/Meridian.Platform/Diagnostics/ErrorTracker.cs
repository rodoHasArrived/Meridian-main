...
{
    private readonly ILogger<ErrorTracker>? _log;
    private readonly ConcurrentQueue<TrackedError> _errors = new();
    private readonly int _maxErrors;
    private readonly string _dataRoot;
    private int _totalErrorCount;

    public ErrorTracker(string dataRoot, int maxErrors = 1000, ILogger<ErrorTracker>? log = null)
    {
        _dataRoot = dataRoot;
        ...
        CancellationToken ct = default)
    {
        ...
        foreach (var logFile in logFiles)
        {
            ...
            {
                _log?.LogDebug(ex, "Log file is locked or inaccessible: {FilePath}", logFile);
            }
        }
