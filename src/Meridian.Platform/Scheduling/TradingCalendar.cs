...
namespace Meridian.Platform.Scheduling;
...
public sealed class TradingCalendar : IOperationalTradingCalendar, ITradingCalendarProvider
{
    ...
    IReadOnlyList<TradingSession> IOperationalTradingCalendar.GetTradingSessions(DateOnly date, string market)
    {
        ...
        var regularClose = IsHalfDay(date) ? HalfDayClose : RegularMarketClose;
        var sessions = new List<TradingSession>
        {
            new(
                ...
                OpenTime: CombineDateTime(date, PreMarketOpen, _easternTimeZone),
                CloseTime: CombineDateTime(date, RegularMarketOpen, _easternTimeZone)),
            new(
                Exchange: "NYSE",
                Market: market,
                SessionType: "regular",
                OpenTime: CombineDateTime(date, RegularMarketOpen, _easternTimeZone),
                CloseTime: CombineDateTime(date, regularClose, _easternTimeZone))
        };
        ...
        if (!IsHalfDay(date))
        {
            sessions.Add(new(
                ...
                OpenTime: CombineDateTime(date, RegularMarketClose, _easternTimeZone),
                CloseTime: CombineDateTime(date, AfterHoursClose, _easternTimeZone)));
        }
