...
public sealed class FundWorkflowCommandHandler
{
    ...
    public FundWorkflowState Handle(RejectWorkflow command) => Apply(command.WorkflowId, command.ReasonCode, command.Metadata);
    ...
    private FundWorkflowState Apply(Guid workflowId, FundWorkflowRejectionReasonCode reason, FundWorkflowCommandMetadata metadata) => Apply(workflowId, metadata, s =>
    {
        Require(s.ApprovalGateOpen, "Workflow must be in approval to reject.");
        RouteRejection(s, reason);
    });
