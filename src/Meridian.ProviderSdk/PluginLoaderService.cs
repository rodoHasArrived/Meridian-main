...
public sealed class PluginLoaderService : IPluginLoaderService
{
    ...
    public async Task<IReadOnlyList<PluginLoadResult>> LoadPluginsAsync(string pluginsDirectory, CancellationToken ct)
    {
        ...
        foreach (var dllPath in dllFiles)
        {
            ...
            if (result.Success)
            {
                _logger.LogInformation(
                    "Loaded plugin {AssemblyPath} — {TypeCount} type(s) registered",
                    dllPath,
                    result.RegisteredTypes.Count);
            }
            else
            {
                _logger.LogWarning(
                    "Plugin {AssemblyPath} failed to load: {ErrorMessage}",
                    dllPath,
                    result.ErrorMessage);
            }
        }
        ...
        _logger.LogInformation(
            "Plugin scan complete: {SuccessCount}/{Total} succeeded",
