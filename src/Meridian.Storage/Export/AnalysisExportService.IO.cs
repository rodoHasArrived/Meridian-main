        ...
        CancellationToken ct)
    {
        ...
        switch (profile.TargetTool.ToLowerInvariant())
        {
            ...
            case "postgresql":
                ...
                break;
            case "runmat":
                scriptPath = Path.Combine(outputDir, "load_data.m");
                script = GenerateRunMatLoader(files, profile);
                break;
            default:
                return string.Empty;
        }
...
""");
    ...
    private string GenerateRunMatLoader(List<ExportedFile> files, ExportProfile profile)
    {
        var exampleSymbol = files.Select(f => f.Symbol)
            .FirstOrDefault(symbol => !string.IsNullOrWhiteSpace(symbol)) ?? "SPY";

        var sb = new StringBuilder();
        sb.AppendLine("% Market Data Loader for RunMat");
        sb.AppendLine($"% Generated by Meridian - {profile.Name}");
        sb.AppendLine("% Numeric CSV layout:");
        sb.AppendLine("%   1 Timestamp (Unix ms)");
        sb.AppendLine("%   2 Price");
        sb.AppendLine("%   3 Size");
        sb.AppendLine("%   4 BidPrice");
        sb.AppendLine("%   5 BidSize");
        sb.AppendLine("%   6 AskPrice");
        sb.AppendLine("%   7 AskSize");
        sb.AppendLine("%   8 Open");
        sb.AppendLine("%   9 High");
        sb.AppendLine("%  10 Low");
        sb.AppendLine("%  11 Close");
        sb.AppendLine("%  12 Volume");
        sb.AppendLine();
        sb.AppendLine("data = load_data();");
        sb.AppendLine("price_series = pick_price_series(data);");
        sb.AppendLine("timestamp_ms = data(:, 1);");
        sb.AppendLine("plot(timestamp_ms, price_series);");
        sb.AppendLine("xlabel('Timestamp (Unix ms)');");
        sb.AppendLine("ylabel('Price');");
        sb.AppendLine("title('Meridian Export Loaded In RunMat');");
        sb.AppendLine();
        sb.AppendLine("% Example:");
        sb.AppendLine($"% symbol_data = load_data('{exampleSymbol}');");
        sb.AppendLine();
        ...
        sb.AppendLine("  end");
        sb.AppendLine();
        sb.AppendLine("  base_dir = fileparts(mfilename('fullpath'));");
        sb.AppendLine("  pattern = '*.csv';");
        sb.AppendLine("  if ~isempty(symbol)");
        sb.AppendLine("    pattern = strcat(symbol, '_*.csv');");
        sb.AppendLine("  end");
        sb.AppendLine();
        sb.AppendLine("  files = dir(fullfile(base_dir, pattern));");
        sb.AppendLine("  if isempty(files)");
        sb.AppendLine("    error('No CSV files found for pattern %s', pattern);");
        sb.AppendLine("  end");
        sb.AppendLine();
        sb.AppendLine("  data = [];");
        sb.AppendLine("  for i = 1:numel(files)");
        sb.AppendLine("    file_path = fullfile(base_dir, files(i).name);");
        sb.AppendLine("    matrix = readmatrix(file_path);");
        sb.AppendLine("    if ~isempty(matrix)");
        sb.AppendLine("      data = [data; matrix];");
        sb.AppendLine("    end");
        sb.AppendLine("  end");
        sb.AppendLine("end");
        sb.AppendLine();
        sb.AppendLine("function series = pick_price_series(data)");
        sb.AppendLine("  if size(data, 2) >= 11 && any(~isnan(data(:, 11)))");
        sb.AppendLine("    series = data(:, 11);");
        sb.AppendLine("  elseif size(data, 2) >= 2 && any(~isnan(data(:, 2)))");
        sb.AppendLine("    series = data(:, 2);");
        sb.AppendLine("  else");
        sb.AppendLine("    error('No price-compatible columns were found in the export.');");
        sb.AppendLine("  end");
        sb.AppendLine("end");
