...
lineage that includes a ledger mapping reference before `ILedgerJournalStore` accepts it. Instrument
account lines must also carry an explicit line symbol, so entry-level Security Master metadata cannot
mask an unattributed securities, receivable, option, futures, or short-position posting.
