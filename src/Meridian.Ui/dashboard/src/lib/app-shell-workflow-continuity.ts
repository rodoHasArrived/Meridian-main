...
import {
  ...
  defaultWorkflowContinuityTrail,
  workflowContinuityTrails,
  type AppShellPrimaryOperatorWorkflowStep,
  type WorkflowContinuityTrailDefinition,
...
export interface AppShellWorkflowContinuityWorkspacePayload {
...
export function buildWorkflowContinuityViewModel(
  ...
  statusContext: AppShellWorkflowContinuityStatusContext = emptyWorkflowContinuityStatusContext,
...
export interface AppShellWorkflowContinuityStatusContext {
  loading: boolean;
  error: string | null;
  workflowError: string | null;
  workspaceErrors: Partial<Record<WorkspaceKey, string>>;
  payload: AppShellWorkflowContinuityWorkspacePayload;
}
...
const emptyWorkflowContinuityStatusContext: AppShellWorkflowContinuityStatusContext = {
  loading: false,
  error: null,
  workflowError: null,
  workspaceErrors: {},
...
function buildWorkflowContinuityStepStatus(
  stepId: string,
  context: AppShellWorkflowContinuityStatusContext
...
function buildTrustedDataContinuityStatus({ payload }: AppShellWorkflowContinuityStatusContext): WorkflowContinuityStepStatus {
...
function buildStrategyContinuityStatus({ payload, workflowError }: AppShellWorkflowContinuityStatusContext): WorkflowContinuityStepStatus {
  if (workflowError) {
...
function buildPaperReadinessContinuityStatus({ payload }: AppShellWorkflowContinuityStatusContext): WorkflowContinuityStepStatus {
...
function buildProviderSetupContinuityStatus({ payload, error, workflowError }: AppShellWorkflowContinuityStatusContext): WorkflowContinuityStepStatus {
...
function buildPortfolioLedgerContinuityStatus({ payload }: AppShellWorkflowContinuityStatusContext): WorkflowContinuityStepStatus {
...
function buildReconciliationContinuityStatus({ payload }: AppShellWorkflowContinuityStatusContext): WorkflowContinuityStepStatus {
...
function buildGovernedReportContinuityStatus({ payload }: AppShellWorkflowContinuityStatusContext): WorkflowContinuityStepStatus {
...
function buildCloseSupportContinuityStatus({ payload }: AppShellWorkflowContinuityStatusContext): WorkflowContinuityStepStatus {
...
function buildWorkflowContinuityDisclosureState(
  statusContext: AppShellWorkflowContinuityStatusContext,
  operatorFocus: OperatorFocusViewModel,
  evidenceTimeline: EvidenceTimelineViewModel,
  linkedContext: LinkedContextViewModel
...
function buildWorkspaceErrorFocusItems(context: AppShellWorkflowContinuityStatusContext): OperatorFocusCandidate[] {
...
function buildTradingFocusItems({ payload }: AppShellWorkflowContinuityStatusContext): OperatorFocusCandidate[] {
  const readiness = payload.trading?.readiness ?? null;
  if (!readiness) {
    return [];
  }
...
function buildDataFocusItems({ payload }: AppShellWorkflowContinuityStatusContext): OperatorFocusCandidate[] {
  const data = payload.data;
  if (!data) {
    return [];
  }
...
function buildPortfolioFocusItems({ payload }: AppShellWorkflowContinuityStatusContext): OperatorFocusCandidate[] {
  const portfolio = payload.portfolio;
...
function buildAccountingFocusItems(context: AppShellWorkflowContinuityStatusContext): OperatorFocusCandidate[] {
...
function buildAccountingWorkflowSummaryFocusItem({ payload }: AppShellWorkflowContinuityStatusContext): OperatorFocusCandidate | null {
...
function buildReviewedAutomationFocusItem({ payload }: AppShellWorkflowContinuityStatusContext): OperatorFocusCandidate | null {
  const summary = getAccountingWorkflowSummary(payload.workflowSummary);
  if (!summary || !isFinancialOperationsWorkflowSummary(summary) || summary.primaryBlocker?.isBlocking) {
    return null;
  }
...
function buildReportingFocusItems({ payload }: AppShellWorkflowContinuityStatusContext): OperatorFocusCandidate[] {
  const reporting = payload.reporting;
  if (!reporting) {
    return [];
  }
...
function buildStrategyFocusItems({ payload }: AppShellWorkflowContinuityStatusContext): OperatorFocusCandidate[] {
  const strategy = payload.strategy;
  if (!strategy) {
    return [];
  }
...
function routeForOperatorWorkItem(item: OperatorWorkItem): string {
  ...
  if (item.kind === "ReportPackApproval") {
    return routeFromOperatorWorkItemTarget(item)
      ?? fallbackRouteForOperatorWorkItemKind(item.kind)
      ?? normalizeLocalWorkstationRoute(item.targetRoute)
      ?? WORKSTATION_ROUTE_CATALOG.reportingGovernance;
  }
...
function fallbackRouteForOperatorWorkItemKind(kind: OperatorWorkItem["kind"]): string {
  switch (kind) {
    ...
    case "ReportPackApproval":
      return WORKSTATION_ROUTE_CATALOG.reportingGovernance;
...
function buildDataLinkedContextItem(
  { payload }: AppShellWorkflowContinuityStatusContext,
  symbol: string
...
function inferPrimaryOperatingSymbol(payload: AppShellWorkflowContinuityWorkspacePayload): string | null {
...
function isSameSymbol(value: string | null | undefined, symbol: string): boolean {
  return normalizeOperatingContextSymbol(value ?? null) === symbol;
}
...
function buildOverviewEvidenceTimelineItems({ payload }: AppShellWorkflowContinuityStatusContext): EvidenceTimelineCandidate[] {
...
function buildTradingEvidenceTimelineItems({ payload }: AppShellWorkflowContinuityStatusContext): EvidenceTimelineCandidate[] {
  const trading = payload.trading;
  if (!trading) {
    return [];
  }
...
function buildDataEvidenceTimelineItems({ payload }: AppShellWorkflowContinuityStatusContext): EvidenceTimelineCandidate[] {
  const data = payload.data;
  if (!data) {
    return [];
  }
...
function buildStrategyEvidenceTimelineItems({ payload }: AppShellWorkflowContinuityStatusContext): EvidenceTimelineCandidate[] {
...
function buildPortfolioEvidenceTimelineItems({ payload }: AppShellWorkflowContinuityStatusContext): EvidenceTimelineCandidate[] {
...
function buildAccountingEvidenceTimelineItems({ payload }: AppShellWorkflowContinuityStatusContext): EvidenceTimelineCandidate[] {
  const accounting = payload.accounting;
  if (!accounting) {
    return [];
  }
