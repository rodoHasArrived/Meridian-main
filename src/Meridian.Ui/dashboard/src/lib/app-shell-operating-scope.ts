...
export function appendSearchValue(route: string, key: string, value: string, preserveExisting = false) {
...
function operatingScopeKeysForRoute(route: string): Set<AppShellOperatingScopeQueryKey> {
  const workspaceKey = workspaceForPath(splitOperatingScopeRoute(route).pathname).key;
  switch (workspaceKey) {
    case "data":
...
function splitOperatingScopeRoute(route: string) {
  const hashIndex = route.indexOf("#");
  const routeWithoutHash = hashIndex >= 0 ? route.slice(0, hashIndex) : route;
  const searchIndex = routeWithoutHash.indexOf("?");
  return {
