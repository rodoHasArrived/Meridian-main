...
describe("workstation API endpoint catalog", () => {
  ...
  it("builds security-master and reconciliation endpoint families from shared roots", () => {
    ...
    expect(reconciliationBreakBulkDryRunEndpoint()).toBe("/api/workstation/reconciliation/break-queue/bulk/dry-run");
    expect(reconciliationBreakBulkExecuteEndpoint()).toBe("/api/workstation/reconciliation/break-queue/bulk/execute");
