...
export function Combobox({
  ...
  return (
    <div ref={rootRef} className={cn("relative w-full", className)}>
      ...
      <div
        className={cn(
          "flex h-9 items-center rounded-[2px] border border-border bg-[var(--surface-input)]",
      ...
      >
        <input
          ...
          className="min-w-0 flex-1 bg-transparent px-2.5 font-sans text-sm text-foreground placeholder:text-muted-foreground/70 focus:outline-none"
        />
      ...
      {open ? (
        <div
          role="listbox"
          id={listboxId}
          className="absolute left-0 right-0 top-[calc(100%+4px)] z-[100] max-h-60 overflow-y-auto rounded-[2px] border border-border bg-popover shadow-[var(--shadow-menu)]"
        >
          {filtered.length === 0 ? (
            ...
            filtered.map((option, index) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "flex cursor-pointer items-center gap-2 px-3 py-2 font-sans text-xs text-foreground",
