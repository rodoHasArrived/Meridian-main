    ...
    <section role="region" aria-label="Daily reporting cockpit" className={cn("workspace-section-band", className)}>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-4">
                  ...
                  <li key={item.workItemId} aria-label={item.ariaLabel} className={cn("grid gap-3 px-3 py-3 lg:grid-cols-[minmax(0,1fr)_auto]", index === 0 && "bg-primary/5")}>
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="text-[11px] font-semibold uppercase text-muted-foreground">{item.kindLabel}</span>
                      ...
                      <dl className="mt-3 grid gap-2 text-xs sm:grid-cols-5" aria-label={`${item.title} decision facts`}>
          ...
          {model.cards.length > 0 ? (
            <section className="rounded-md border border-border/70 bg-background/75" aria-label="Report family health">
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border/70 px-3 py-2">
                <div className="flex items-center gap-2">
                  <TableProperties className="h-4 w-4 text-primary" aria-hidden="true" />
                  <h3 className="text-sm font-semibold text-foreground">Report family health</h3>
                </div>
                ...
                <table className="min-w-full text-sm">
                  ...
                  <tbody>
                    {model.cards.map((card) => (
                      <tr key={card.family} aria-label={card.ariaLabel} className="border-t border-border/70">
        ...
        <aside className="rounded-md border border-border/70 bg-background/75 p-3" aria-label="Selected reporting work detail">
          ...
          {selectedWork ? (
            ...
            <p className="mt-3 text-sm leading-6 text-muted-foreground">
              No reporting work is selected. Report family health remains available for routine review.
            </p>
          )}
        </aside>
      </div>
