...
describe("command palette view model", () => {
  it("marks the current workspace and canonical guided setup route", () => {
    ...
    expect(model.itemCountLabel).toBe("7 workspaces - 19 quick routes");
    expect(model.commandListLabel).toBe("26 workstation commands");
    expect(model.filteredItemCountLabel).toBe("26 commands available");
    expect(model.commandGroups.map((group) => `${group.label}:${group.countLabel}`)).toEqual([
      "Workspaces:7 workspaces",
      "Quick routes:19 quick routes"
    ]);
    ...
    expect(model.filteredItems).toHaveLength(26);
  });
  ...
  it("filters commands by workspace, route, status, and description text", () => {
    ...
    expect(model.filteredItemCountLabel).toBe("1 of 26 commands match");
  ...
  it("groups entity search results ahead of workspace routes", () => {
    ...
    expect(model.itemCountLabel).toBe("1 entity result - 7 workspaces - 19 quick routes");
  ...
  it("promotes ranked operator focus actions ahead of route navigation", () => {
    ...
    expect(model.itemCountLabel).toBe("2 focus actions - 7 workspaces - 19 quick routes");
    expect(model.commandListLabel).toBe("28 workstation commands");
  ...
  it("exposes a searchable empty state when commands do not match", () => {
    ...
    expect(model.items).toHaveLength(26);
    expect(model.filteredItems).toEqual([]);
    expect(model.commandGroups).toEqual([]);
    expect(model.filteredItemCountLabel).toBe("0 of 26 commands match");
  ...
  it("keeps quick routes available when workspace metadata is missing", () => {
    ...
    expect(model.items).toHaveLength(19);
    expect(model.commandListLabel).toBe("19 workstation commands");
    expect(model.itemCountLabel).toBe("0 workspaces - 19 quick routes");
  ...
  it("exposes focused portfolio, accounting, reporting, strategy, and data route commands", () => {
    ...
    // Family Office and the Formula Workbench are registered in UNWIRED_WORKSTATION_ROUTES: both
    // render a permanent "not connected" state, so the palette must not offer them even when the
    // operator is standing on the route. Steering someone into a guaranteed dead end is worse than
    // omitting the command.
    expect(familyOfficeModel.filteredItems.map((item) => item.id)).not.toContain("route:portfolio-family-office");
    expect(familyOfficeModel.items.find((item) => item.id === "route:portfolio-family-office")).toBeUndefined();
  ...
  it("adds backend workflow actions and pinned presets to command routing", () => {
    ...
    expect(model.itemCountLabel).toBe("7 workspaces - 19 quick routes - 1 preset - 3 workflow actions");
    expect(model.commandListLabel).toBe("30 commands");
