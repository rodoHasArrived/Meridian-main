  ...
  it("derives Security Master master-detail page summary from selected state", () => {
    ...
    expect(state.metrics).toEqual(expect.arrayContaining([
      expect.objectContaining({
        id: "selected",
        value: "Apple Inc.",
        detail: "Security ID sec-1; Equity; Active.",
        tone: "success"
      }),
      expect.objectContaining({ id: "conflicts", value: "1", tone: "warning" }),
      expect.objectContaining({ id: "reference", label: "Reference coverage" }),
      expect.objectContaining({ id: "passport", label: "Passport controls" })
    ]));
