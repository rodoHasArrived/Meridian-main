    ...
    <Card className="panel-surface" role="region" aria-labelledby="capital-account-workbench-heading">
      ...
      <CardContent className="space-y-5">
        ...
        <div className="flex flex-wrap gap-2">
          <AccountingWorkbenchChip label="Projected" value={view.projectedAtLabel} />
          <AccountingWorkbenchChip label="Route" value={view.workbenchRouteLabel} />
        </div>
              ...
              {view.allocationRules.map((row) => (
                <div key={row.id} className={cn("rounded-md border px-3 py-2 text-sm", accountingToolingBorderClass(row.statusTone))}>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-xs">
                    <AccountingWorkbenchChip label="Evidence" value={row.evidenceLabel} />
                    <AccountingWorkbenchChip label="Required" value={row.requiredLabel} />
                    <AccountingWorkbenchChip label="Policy" value={row.policyLabel} />
                    <AccountingWorkbenchChip label="Effective" value={row.effectiveWindowLabel} />
                    <AccountingWorkbenchChip label="Approval" value={row.approvalLabel} />
                    <AccountingWorkbenchChip label="Inputs" value={row.inputSummaryLabel} />
                    <AccountingWorkbenchChip label="Fund events" value={row.relatedFundEventLabel} />
                  </div>
            ...
            <div className="grid gap-2">
              {view.auditDrillThroughs.map((row) => (
                <div key={row.id} className={cn("rounded-md border px-3 py-2 text-sm", accountingToolingBorderClass(row.statusTone))}>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-xs">
                    <AccountingWorkbenchChip label="Evidence" value={row.evidenceLabel} />
                    <AccountingWorkbenchChip label="Related" value={row.relatedLabel} />
                  </div>
                  {row.routeLabel !== "No route" ? (
                    <a href={row.routeLabel} className="mt-2 block break-all font-mono text-[11px] text-primary hover:underline">
                      {row.routeLabel}
...
function AccountingWorkbenchChip({ label, value }: { label: string; value: string }) {
  return (
