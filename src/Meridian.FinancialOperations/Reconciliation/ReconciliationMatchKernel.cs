...
public static class ReconciliationMatchKernel
{
    ...
    /// deterministic: smallest absolute residual, then fewest legs, then lexicographically smallest
    /// leg-id sequence. A split needs at least two legs — single-leg matches belong to the pair
    /// stages. Candidates whose sign differs from the target are ignored (netting mixed-sign groups
    /// is casework, not floor matching), and only the <see cref="MaxSplitSearchCandidates"/> largest
    /// candidates by magnitude participate.
    /// </summary>
        ...
        out decimal residual)
    {
        ...
        void Search(int index, decimal sumAbs)
        {
            if (current.Count >= 2)
            {
                ...
                if (residualAbs <= tolerance && (!bestFound
        ...
        int CompareLegIds(List<int> left, List<int> right)
        {
            ...
            {
                var compared = string.CompareOrdinal(pool[left[i]].Id, pool[right[i]].Id);
                if (compared != 0)
                {
                    return compared;
                }
