...
public sealed class AccountingPostingCandidatePostService : IAccountingPostingCandidatePostService
{
        ...
        CancellationToken ct)
    {
        ...
        if (instruction.Intent == AssetLotMutationIntentDto.Acquire)
        {
            ...
            if ((!acquisition.HasFaceValueTerms || faceTerms is not null) &&
                !await IsRetainedLegacyAcquisitionBatchAsync(mutationBatchId, ct).ConfigureAwait(false))
            {
                acquisitionLot = acquisitionLot with
                {
                    Acquisition = BuildAcquisitionFacts(
                        acquisitionLot,
                        faceTerms,
    ...
    private static OpenLotAcquisitionDto BuildAcquisitionFacts(
    ...
    private async Task<bool> IsRetainedLegacyAcquisitionBatchAsync(Guid mutationBatchId, CancellationToken ct)
    {
        ...
        try
        {
            retained = await _journalStore.GetAtomicTaxLotPostingAsync(mutationBatchId, ct).ConfigureAwait(false);
        }
