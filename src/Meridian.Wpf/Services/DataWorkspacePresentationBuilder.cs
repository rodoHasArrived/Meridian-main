...
public static class DataWorkspacePresentationBuilder
{
    ...
    public static DataWorkspacePresentation Build(DataWorkspaceData data)
    {
        ...
        var heroMetrics = BuildHeroMetrics(
            ...
            isFixtureMode);
        var integrationWorkflowSteps = BuildIntegrationWorkflowSteps(
            data,
            providerCount,
            healthyProviderCount,
            providersTone,
        ...
        return new DataWorkspacePresentation
        {
            ...
            IntegrationWorkflowSteps = integrationWorkflowSteps,
    ...
    private static DataHeroState BuildHeroState(
        DataWorkspaceData data,
    ...
    private static DataHeroState CreateHeroState(
        string focusText,
        string summaryText,
        string badgeText,
        string badgeTone,
    ...
    private static IReadOnlyList<DataHeroMetric> BuildHeroMetrics(
        int providerCount,
        int healthyProviderCount,
    ...
    private static IReadOnlyList<DataIntegrationWorkflowStep> BuildIntegrationWorkflowSteps(
        DataWorkspaceData data,
        int providerCount,
        int healthyProviderCount,
        string providersTone,
        ...
        string storageTone,
        int resumableCount,
        BackfillExecution? latestExecution,
        ExportJob? latestExportJob,
        int criticalStorageIssueCount,
        int exportRunningCount,
        int exportFailedCount,
        bool isFixtureMode)
    {
        var hasConnectedProvider = providerCount > 0
            && healthyProviderCount >= providerCount
            && data.ProviderStatus?.IsConnected == true;
        var hasProviderPosture = isFixtureMode || hasConnectedProvider;
        var hasHistoricalCoverage = resumableCount > 0
            || latestExecution is not null
            || data.LastBackfillStatus is not null
            || data.BackfillSchedules.Any(schedule => schedule.IsEnabled);
        var backfillNeedsReview = data.LastBackfillStatus is { Success: false }
            || IsFailedExecution(latestExecution);
        var hasStoredData = data.StorageStats is not null
            && criticalStorageIssueCount == 0;
        var hasPublishActivity = latestExportJob is not null;
        ...
        [
            CreateWorkflowStep(
                "connect-source",
                "Connect Source",
                "Provider credentials, entitlements, and routing posture are confirmed before collection starts.",
                hasConnectedProvider ? WorkflowStepStatus.Complete : WorkflowStepStatus.Current,
                hasConnectedProvider ? "Connected" : isFixtureMode ? "Demo" : providerCount == 0 || providersTone == WorkspaceTone.Danger ? "Blocked" : "Review"),
            CreateWorkflowStep(
                "acquire-data",
                "Acquire Data",
                "Live collection sessions and historical backfills acquire source data for the active scope.",
                hasHistoricalCoverage && !backfillNeedsReview
                    ? resumableCount > 0 ? WorkflowStepStatus.Current : WorkflowStepStatus.Complete
                    : hasProviderPosture ? WorkflowStepStatus.Current : WorkflowStepStatus.Pending,
                resumableCount > 0 ? "In progress" : backfillNeedsReview ? "Review" : hasHistoricalCoverage ? "Ready" : hasProviderPosture ? "Next" : "Pending"),
            CreateWorkflowStep(
                "validate-data",
                "Validate Data",
                "Backfill results, provider health, and storage diagnostics provide evidence before promotion.",
                backfillNeedsReview ? WorkflowStepStatus.Current : hasHistoricalCoverage ? WorkflowStepStatus.Complete : WorkflowStepStatus.Pending,
                backfillNeedsReview ? "Review" : hasHistoricalCoverage ? "Checked" : "Pending"),
            CreateWorkflowStep(
                "normalize-data",
                "Normalize Data",
                "Provider-specific payloads are normalized into shared contracts for replay, research, and books.",
                hasStoredData ? WorkflowStepStatus.Complete : hasHistoricalCoverage ? WorkflowStepStatus.Current : WorkflowStepStatus.Pending,
                hasStoredData ? "Normalized" : hasHistoricalCoverage ? "Next" : "Pending"),
            CreateWorkflowStep(
                "store-data",
                "Store Data",
                "Accepted market and operations data is durably stored with storage-health evidence.",
                criticalStorageIssueCount > 0 ? WorkflowStepStatus.Current : hasStoredData ? WorkflowStepStatus.Complete : WorkflowStepStatus.Pending,
                criticalStorageIssueCount > 0 ? "Blocked" : hasStoredData ? "Stored" : storageTone == WorkspaceTone.Warning ? "Review" : "Pending"),
            CreateWorkflowStep(
                "publish-data",
                "Publish Data",
                "Export packages and downstream handoffs publish governed data for operators and reports.",
                exportRunningCount > 0 || exportFailedCount > 0
                    ? WorkflowStepStatus.Current
                    : hasPublishActivity ? WorkflowStepStatus.Complete : WorkflowStepStatus.Pending,
                exportRunningCount > 0 ? "Publishing" : exportFailedCount > 0 ? "Review" : hasPublishActivity ? "Published" : "Pending")
        ];
    }

    private static DataIntegrationWorkflowStep CreateWorkflowStep(
        string id,
        string label,
        string description,
        string status,
        string statusLabel)
    {
        ...
        {
            WorkflowStepStatus.Complete => WorkspaceTone.Success,
            WorkflowStepStatus.Current => WorkspaceTone.Info,
            _ => WorkspaceTone.Neutral
        };

        if (status == WorkflowStepStatus.Current
            && (string.Equals(statusLabel, "Blocked", StringComparison.OrdinalIgnoreCase)
                || string.Equals(statusLabel, "Review", StringComparison.OrdinalIgnoreCase)))
        {
            tone = WorkspaceTone.Warning;
        }

        return new DataIntegrationWorkflowStep
        {
            Id = id,
            Label = label,
    ...
    private static WorkspaceQueueItem BuildProviderItem(DataWorkspaceData data, int providerCount, int healthyProviderCount, string tone)
    {
    ...
    private static WorkspaceQueueItem BuildBackfillItem(DataWorkspaceData data, BackfillExecution? latestExecution, int resumableCount, int activeResumables, int pendingSymbols, int enabledSchedules)
    {
    ...
    private static IReadOnlyList<WorkspaceRecentItem> BuildRecentOperations(DataWorkspaceData data, BackfillExecution? latestExecution, CollectionSession? latestSession, ExportJob? latestExportJob)
    {
    ...
    private static int GetProviderCount(DataWorkspaceData data) => Math.Max(Math.Max(data.Providers.Count, data.ProviderStatus?.AvailableProviders.Count ?? 0), data.BackfillHealth?.Providers?.Count ?? 0);
    private static int GetHealthyProviderCount(DataWorkspaceData data, int providerCount) => providerCount == 0 ? 0 : Math.Min(providerCount, Math.Max(data.BackfillHealth?.Providers?.Count(entry => entry.Value.IsAvailable) ?? 0, data.ProviderStatus?.IsConnected == true ? 1 : 0));
...
public sealed class DataWorkspaceData
{
...
public sealed class DataWorkspacePresentation
{
    public WorkspaceShellContextInput Context { get; init; } = new();
    public WorkspaceCommandGroup CommandGroup { get; init; } = new();
    public DataHeroState HeroState { get; init; } = DataHeroState.Loading();
    public IReadOnlyList<DataHeroMetric> HeroMetrics { get; init; } = DataHeroMetric.LoadingMetrics();
    public IReadOnlyList<DataIntegrationWorkflowStep> IntegrationWorkflowSteps { get; init; } = DataIntegrationWorkflowStep.LoadingSteps();
    ...
    public string SummaryProvidersText { get; init; } = DataWorkspacePresentationBuilder.ProvidersUnavailableSummary;
    public string SummaryProvidersTone { get; init; } = WorkspaceTone.Warning;
    public string SummaryBackfillText { get; init; } = DataWorkspacePresentationBuilder.BackfillUnavailableSummary;
    public string SummaryBackfillTone { get; init; } = WorkspaceTone.Neutral;
    public string SummaryStorageText { get; init; } = DataWorkspacePresentationBuilder.StorageUnavailableSummary;
...
public sealed class DataHeroState
{
    ...
    public static DataHeroState Loading() =>
        new();

    public static DataHeroState Error() =>
        new()
        {
            FocusText = "Operational posture",
...
public sealed class DataHeroMetric
{
    ...
    public static IReadOnlyList<DataHeroMetric> LoadingMetrics() =>
    [
        new()
        {
            Label = "Providers",
    ...
    public static IReadOnlyList<DataHeroMetric> ErrorMetrics() =>
    [
        new()
        {
            Label = "Providers",
...
public sealed class DataIntegrationWorkflowStep
{
    public string Id { get; init; } = string.Empty;
    public string Label { get; init; } = string.Empty;
    public string Description { get; init; } = string.Empty;
    public string Status { get; init; } = WorkflowStepStatus.Pending;
    public string StatusLabel { get; init; } = "Pending";
    public string Tone { get; init; } = WorkspaceTone.Neutral;
    public string AutomationName => $"{Label}: {StatusLabel}";

    public static IReadOnlyList<DataIntegrationWorkflowStep> LoadingSteps() =>
    [
        new()
        {
            Id = "connect-source",
            Label = "Connect Source",
            Description = "Refreshing provider connectivity and credential posture.",
        ...
        new()
        {
            Id = "acquire-data",
            Label = "Acquire Data",
            Description = "Waiting for provider readiness before acquisition starts.",
        ...
        new()
        {
            Id = "validate-data",
            Label = "Validate Data",
            Description = "Validation evidence appears after acquired data is available.",
            Status = WorkflowStepStatus.Pending,
            StatusLabel = "Pending",
            Tone = WorkspaceTone.Neutral
        },
        new()
        {
            ...
            Description = "Normalization follows validated provider payloads.",
            Status = WorkflowStepStatus.Pending,
            StatusLabel = "Pending",
            Tone = WorkspaceTone.Neutral
        },
        new()
        {
            Id = "store-data",
            Label = "Store Data",
            Description = "Storage evidence appears after normalized data is accepted.",
            Status = WorkflowStepStatus.Pending,
            StatusLabel = "Pending",
            Tone = WorkspaceTone.Neutral
        },
        new()
        {
            Id = "publish-data",
            Label = "Publish Data",
            Description = "Export and report handoffs remain pending until stored data is packaged.",
            Status = WorkflowStepStatus.Pending,
            StatusLabel = "Pending",
            Tone = WorkspaceTone.Neutral
        }
    ...
    public static IReadOnlyList<DataIntegrationWorkflowStep> ErrorSteps() =>
    [
        new()
        {
            Id = "connect-source",
            Label = "Connect Source",
            Description = "Provider telemetry could not be refreshed.",
            Status = WorkflowStepStatus.Current,
            StatusLabel = "Review",
            Tone = WorkspaceTone.Warning
        },
