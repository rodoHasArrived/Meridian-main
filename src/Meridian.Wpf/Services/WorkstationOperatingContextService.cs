        ...
        foreach (var business in graph.Businesses)
        {
            ...
            contexts[WorkstationOperatingContext.CreateContextKey(OperatingContextScopeKind.Business, business.BusinessId.ToString("D"))] =
                new WorkstationOperatingContext
                {
                    ...
                    DefaultWorkspaceId = WorkstationNavigationDefaults.StrategyWorkspaceId,
                    DefaultLandingPageTag = WorkstationNavigationDefaults.StrategyPageTag,
        ...
        CancellationToken ct)
    {
            ...
            .ThenBy(static mapping => mapping.LaneName, StringComparer.OrdinalIgnoreCase))
        {
            ...
            contexts[mapping.ContextKey] = context with
            {
                ...
                DefaultWorkspaceId = WorkstationNavigationDefaults.NormalizeWorkspaceId(lane.DefaultWorkspaceId),
                DefaultLandingPageTag = WorkstationNavigationDefaults.NormalizePageTag(lane.DefaultLandingPageTag)
            };
        }
    ...
    private static IEnumerable<WorkstationOperatingContext> BuildFundProfileContexts(IReadOnlyList<FundProfileDetail> profiles)
    {
        foreach (var profile in profiles)
        {
            yield return new WorkstationOperatingContext
            {
                ...
                DefaultWorkspaceId = WorkstationNavigationDefaults.NormalizeWorkspaceId(
                    profile.DefaultWorkspaceId,
                    WorkstationNavigationDefaults.AccountingWorkspaceId),
                DefaultLandingPageTag = WorkstationNavigationDefaults.NormalizePageTag(
