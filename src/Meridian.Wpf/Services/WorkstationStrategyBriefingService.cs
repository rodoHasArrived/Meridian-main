...
public interface IWorkstationStrategyBriefingApiClient
{
    Task<StrategyBriefingDto?> GetBriefingAsync(CancellationToken ct = default);
}

public sealed class WorkstationStrategyBriefingApiClient : IWorkstationStrategyBriefingApiClient
{
    private readonly ApiClientService _apiClient;

    public WorkstationStrategyBriefingApiClient(ApiClientService apiClient)
    {
...
public sealed class StrategyBriefingWorkspaceService : IStrategyBriefingWorkspaceService
{
    private readonly IWorkstationStrategyBriefingApiClient _apiClient;
    private readonly StrategyRunWorkspaceService _runService;
    private readonly IWatchlistReader _watchlistReader;

    public StrategyBriefingWorkspaceService(
        IWorkstationStrategyBriefingApiClient apiClient,
    ...
    private async Task<StrategyBriefingDto> BuildFallbackAsync(
    ...
    private static IReadOnlyList<StrategyBriefingAlert> BuildAlerts(
        IReadOnlyList<StrategyRunSummary> runs,
        IReadOnlyList<StrategyRunDetail?> details)
    {
    ...
    private static IReadOnlyList<StrategyWhatChangedItem> BuildWhatChanged(IReadOnlyList<StrategyRunSummary> runs)
        => runs
            .Take(4)
    ...
    private static StrategyRunDrillInLinks BuildDrillInLinks(StrategyRunSummary run)
        => new(
