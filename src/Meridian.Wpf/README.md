...
`RetentionAssurance` now shows a cleanup readiness card and inline confirmation for destructive cleanup. `RetentionAssuranceViewModel` owns guardrail inputs, retention-policy validation, dry-run result state, busy/error copy, and cleanup command gating while the page keeps legal-hold and audit-list rendering in WPF.
...
`RetentionAssurance` includes view-model-owned cleanup readiness, validation results, dry-run staging, inline confirmation, and execute gating so retention cleanup cannot be triggered from stale or invalid policy inputs.
