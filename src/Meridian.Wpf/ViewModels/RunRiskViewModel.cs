...
using ContractsEquityCurvePoint = Meridian.Contracts.Workstation.EquityCurvePoint;
...
public sealed class RunRiskViewModel : BindableBase
{
    ...
    /// </summary>
    private static PlotRequest BuildVolatilityPlot(IReadOnlyList<ContractsEquityCurvePoint> points, int window = 21)
    {
    ...
    private static double ComputeRollingWindow(IReadOnlyList<ContractsEquityCurvePoint> points, int endIndex, int window)
    {
    ...
    /// </summary>
    private static double ComputeAnnualisedVolatility(IReadOnlyList<ContractsEquityCurvePoint> points)
    {
        if (points.Count < 2)
            return double.NaN;
